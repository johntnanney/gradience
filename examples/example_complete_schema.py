...
def demonstrate_complete_schema():
    ...
    if analysis["flagged_layers"]:
        ...
        print("     - Energy share: meaningful fraction ✓")
    ...
    print("   • Algorithm name tracks implementation changes")
...
def show_comparison_scenario():
    ...
    print(
        f"  Configuration: {analysis_a['disagreement_config']['quantile_threshold']:.1f} quantile, {analysis_a['disagreement_config']['uniform_mult_gate']:.1f}× gate"
    )
    ...
    print("\nTEAM B RESULTS:")
    print(
        f"  Configuration: {analysis_b['disagreement_config']['quantile_threshold']:.1f} quantile, {analysis_b['disagreement_config']['uniform_mult_gate']:.1f}× gate"
    )
    ...
    print("   Without schema versioning: 'Why do we get different answers?'")
    print("   With schema versioning: 'Team A used conservative thresholds, Team B used aggressive'")
    print("   → Clear understanding, no debugging mystery")
