...
def demonstrate_focus_consumption():
    ...
    audit_result = {
        ...
        "disagreement_focus_set": {
            "high_impact_layers": ["model.layers.0.self_attn.q_proj", "model.layers.1.mlp.up_proj"],
    ...
    focused_rank_pattern = {
        layer: full_rank_pattern[layer] for layer in high_impact_layers if layer in full_rank_pattern
    }
    ...
    print("  • Validation complexity: HIGH (test every layer)")
    print("  • Risk: Waste time on unimportant layers")

    print("\\nFocus-based approach:")
    print(f"  • Layers tested: {len(focused_rank_pattern)}")
    print("  • Validation complexity: LOW (test only critical layers)")
    print("  • Benefit: Focus effort where it matters most")
    print(
        f"  • Reduction: {((len(original_ranks) - len(focused_rank_pattern)) / len(original_ranks)) * 100:.0f}% fewer layers to validate"
    )
