    ...
    if all_layers:
        ...
        if layers_with_scores:
            sorted_layers = sorted(
                layers_with_scores, key=lambda x: x["flagging_rationale"]["priority_score"], reverse=True
            )
...
def format_threshold_status(value: float, threshold: float | None, name: str) -> str:
...
def print_validation_plan(layers: list[dict[str, Any]], audit_data: dict[str, Any]):
    ...
    for i, layer in enumerate(layers, 1):
        ...
        # Threshold analysis
        spread_status = format_threshold_status(metrics["spread"], metrics["spread_threshold"], "spread")
...
def generate_bench_config(layers: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    config = {"compression_strategies": []}

    # Add uniform strategy
    config["compression_strategies"].append(
        {"variant": "uniform", "rank": uniform_fallback_rank, "description": "Conservative uniform baseline"}
    )
...
def main():
    ...
    parser.add_argument(
        "--top-n", type=int, default=3, help="Fallback to top-N layers if no flagged layers (default: 3)"
    )
