...
### Spectral observables hierarchy (bounded)
...
Policy:
- keep both probes in the research stack as secondary observables
- do not treat them as front-line summary metrics
- do not over-interpret probe-only movement without core-observable support
...
**Route 2 Claims Stability Ladder** (n108-n112) is complete and now reinforced by follow-on replication updates. This synthesis program graded 20 Route 2 claims across evidence base, perturbation survival, artifact coverage, behavioral grounding, and product relevance. **Ladder distribution:** 11 `stable`, 7 `moderately_stable`, 2 `thin`, 0 `local_only`, 0 `blocked_or_open`. Stable cluster remains dominated by workflow invariants and aggregation seam claims; most guarded claims remain middle-state portability and scenario transfer boundaries. Results: `docs/strategy/route2_claims_ladder_summary.md`. Structured outputs: `sidecar/results/route2_claims_ladder/`. Notes: n108-n112.

**Route 2 Mixed-Evidence Triage Stress Test** (n113-n117) is complete. This follow-on stress pass intentionally over-weighted mixed-evidence review and same-family optional cases to test whether the practically important triage middle remains structured. **Verdicts:** QA-dominant coherence `coherent`; same-family optional safe-likeness, triage-middle structure, and structural nuance inside mixed evidence `coherent_with_guardrails`; 0 ambiguous; 0 weakened. Results: `docs/strategy/mixed_evidence_triage_summary.md`. Structured outputs: `sidecar/results/route2_stress_tests/mixed_evidence_triage/`. Notes: n113-n117.

**Route 2 Collapse vs Contamination Replication** (n118-n122) is complete. This targeted replication tested whether one of the strongest behavioral distinctions (uncertainty-dominant collapse vs confident contamination) survives nearby case/slice perturbation. **Verdict:** `replicated_with_guardrails`. Confidence-channel separation remained strong (collapse-rate ratio ~10.3x; confident-wrong ratio ~11.5x in the opposite direction), while neither-source pressure remained similar across channels. Scope remains bounded to merge-facing settings and current case lineage. Results: `docs/strategy/collapse_vs_contamination_summary.md`. Structured outputs: `sidecar/results/route2_stress_tests/collapse_vs_contamination/`. Notes: n118-n122.
...
Total assets: 121 notes, 1 study, 17 analysis scripts, 126 structured data outputs, 69 figures, 1 dossier template, 1 extended case table, 2 research packets.
