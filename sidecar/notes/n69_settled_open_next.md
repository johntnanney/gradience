...
**Collapse and contamination are distinct operational failure channels (replicated, bounded).** Worst-case collapse (fragile merges) and cross-task contamination produce similar neither-source pressure but through opposite confidence patterns: collapse-like targets remain confidence-collapse-heavy while contamination-like targets remain high-confidence-wrong-heavy. The nearby replication pass (n118-n122) strengthened this distinction under case/slice perturbation while keeping scope bounded to merge-facing settings and current case lineage. This behaviorally justifies decision-context-dependent aggregation — different aggregation families track different failure channels.
...
**Cross-task contamination breadth.** The confident-contamination pathway now has bounded replication support in the CT-01 case family (slice perturbations), but broader case-family coverage is still open. Whether this generalizes to additional cross-task pairs with different task combinations remains unresolved.
