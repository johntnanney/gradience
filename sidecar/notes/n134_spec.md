# N134 — Decoder-Scale Controlled Merge Triage (Pre-Registration, v3)
...
**Version history.**
- v1 (drafted pre-literature-review): four-confound design, three-method scheduled comparison (Gradience / KnOTS / TSV).
...
**C1 — Source-metric dynamic range.** In N133, four of six source accuracies saturated at ceiling (≥ 0.97) and one sat near floor (GSM8K, 0.23–0.32). Degradation was bounded below by measurement-range effects independent of merge geometry. **N134 constraint:** every source adapter must achieve accuracy in the range **[0.70, 0.90]** on its own held-out validation set. Adapters falling outside this band are re-trained at adjusted rank or data fraction until they land inside it; adapters that cannot be brought into the band after two attempts are excluded and the task replaced from the reserve list (§3.2).

**C2 — Task-family non-partition.** In N133, six tasks partitioned into two coarse families (discriminative with headroom vs. generative near ceiling), and alignment-based scores operated as family classifiers. **N134 constraint:** ≥ 5 distinct task families represented, with no family containing more than ~25% of tasks. Tasks must be pilot-tested for adapter erank and required to span a continuous erank distribution (no binary high/low split). Family labels are fixed before training; no post-hoc regrouping.

**C3 — Within-task variance.** In N133, 2 seeds per task yielded 1 same-task pair per task — a 6-observation estimate of same-task alignment variance. **N134 constraint:** **≥ 3 seeds per task**, yielding C(3,2) = 3 same-task pairs per task as the minimum for within-task variance estimation.

**C4 — No post-hoc fitting.** In N133, ten composite risk scores were constructed and tested after the primary B-P5 failed. Three matched the 3/3 baseline recall by tiebreak; none survived diagnostic confound checks. **N134 constraint:** the primary H1 score is specified in §4 with a single decision rule. Any secondary or exploratory scores are labeled as such, reported with their own independent thresholds, and carry no evidential weight for the primary question. Scores not pre-registered here cannot be computed.

## 3. Experimental Design

### 3.1 Model

**Mistral-7B-v0.3** (HuggingFace: `mistralai/Mistral-7B-v0.3`), same checkpoint as N133. Same-backbone design is deliberate: the primary question is whether the N133 null survives under unconfounded task selection, not whether it generalizes to a new decoder. A second decoder (Llama-3-8B) is scheduled as N135 conditional on the N134 H1 outcome.
...
- **45 cross-task pairs** evaluated: 3 pairs drawn from each of the C(8,2) = 28 cross-task-type cells. Because 3 × 28 = 84 > 45, not all cells get 3 observations; cells are sampled in order using the committed RNG (`N134_PAIR_SEED = 134`) until 45 pairs are drawn, with a minimum of 1 pair per cell enforced (28 minimum), and the remaining 17 pairs distributed to cells selected by the same RNG without replacement.

This is roughly 4× N133's evaluated cross-task budget (12 pairs) and gives each cross-task-type cell ≥ 1 observation.
...
**This is a hard requirement.** N133's Phase-2 audit persisted only scalar SV-weighted alignment and per-layer singular values, which blocked the scheduled KnOTS / TSV / SVC comparison. N134 **must** persist per-layer U and V orthonormal factors for every LoRA layer.

Required fields in `sidecar/data/n134/audit_v2_1.schema.json`:
...
**Rationale.** N133's Phase-2 finding that O-projection alone gives 7.23× same/cross separation (vs. 3.06× aggregate) and the bonus layer-depth trend showing same/cross ratio rising from 2.32× at layer 0 to 4.24× at layer 31. The score encodes both findings. Crucially, N133's B-P5.b diagnostic showed this score **flipped sign** between raw and family-residualized correlations on the N133 task set — meaning its apparent signal was family-carried. If O-depth clears H1 on N134's unconfounded task set, the flip disappears and the score is validated. If it does not, the hypothesis that per-module weighting recovers signal is retired.

**Outcome variable:** `max_degradation`, defined as $\max(s_A - m_A, s_B - m_B)$ where $s_A, s_B$ are source accuracies on their own tasks and $m_A, m_B$ are the merged model's accuracies evaluated on task A and task B respectively, under 0.5/0.5 linear merge. Consistent with N133.
...
- Within-module C_k → alignment partial correlation (further replication of N132/N133 null).
- Layer-depth trend in same/cross ratio (replication of N133 bonus).
- Dump of all 10 N133 composite risk scores evaluated on N134 data, for comparison purposes only.
...
- **Bootstrap:** block bootstrap for confidence intervals on H1's partial $\rho$, with blocks defined by task-family-pair cell (respecting the dependence structure that created N133's Simpson's paradox). **5,000 resamples.**
...
**Pre-registered direction:** "Gradience is at least as good as KnOTS, TSV, and SVC on the triage objective on this unconfounded task set." This prediction is descriptive — it does not gate H1, and the primary H1 decision stands regardless of this comparison's outcome.
...
**Not included:** TARA-Merging (requires preference signal / pseudo-loss evaluation; fundamentally different intervention class, not a fair triage comparison). Core Space Merging (operationally similar to KnOTS; added to v2.0 integration roadmap, not to N134).

**External citation:** OSRM (Zhang & Zhou, ACL 2025, arXiv:2505.22934) is cited in the N134 report as a training-time intervention that extends the KnOTS observation that weight-space subspace orthogonality does not guarantee non-interference. OSRM is not comparable as a triage method on pre-existing adapters. Its relevance is conceptual: it motivates a future *activation-informed* overlap diagnostic that is explicitly out of scope for N134.
...
This is strict, and deliberately so. The N133 post-hoc composite search is the failure mode N134 is designed to make impossible.

## 8. What Each Outcome Means

**H1 clears, replications hold.** The spectral-triage-for-per-pair-prediction hypothesis is confirmed at decoder scale on an unconfounded task set. Gradience's operational claim generalizes beyond task-boundary detection. N135 (cross-decoder replication on Llama-3-8B) is licensed. The four-method comparison in §6 situates Gradience relative to the execution-method literature.

**H1 fails, replications hold.** The spectral pipeline's validated capability at decoder scale is task-boundary detection (B-P1 / B-P2), not per-pair risk prediction. Gradience's decoder-scale product surface narrows accordingly. The research question shifts: what *does* predict per-pair decoder merge outcome inside a family? Four candidates are pre-specified here so that post-hoc explanation is disciplined:

1. **Source-adapter behavioral quality.** Study 16 (Llama-2-7B, §9 of FINDINGS.md) showed that structural compatibility is necessary but not sufficient; Badirli et al. (arXiv:2602.12323) replicated this at HuggingFace-ecosystem scale. Quality may be the binding constraint at decoder scale rather than pairwise geometry.
...
3. **Task-family identity as the dominant signal.** N133's diagnostic showed task-family identity alone explained R² = 0.97 of cross-task merge outcomes under FAMILY_B. If this holds under N134's unconfounded task set, it would indicate that geometric pair-specificity is not reachable at this scale with weight-space measurement; operational triage would reduce to task-family lookup.

4. **Intrinsic mergeability (Rahamim et al. 2026).** If adapter mergeability is primarily a scalar property of each individual adapter (governed by base-model prior knowledge) rather than a pairwise property, then no pairwise score can succeed in principle at this scale. This hypothesis is directly testable by measuring within-adapter consistency of merge outcomes across partners on N134's evaluated-pair sample, as a supplementary analysis.

Distinguishing these candidates is the N135 question, not N134's. N134 tells us whether the weight-space pairwise-spectral hypothesis survives; it does not adjudicate among these four alternatives in the event of failure.

**Replications fail.** Task-boundary detection does not replicate on the N134 task set. This would be highly surprising and would require diagnosis before any further decoder-scale work proceeds. Most likely cause: the [0.70, 0.90] accuracy band selected tasks that are structurally similar in some way the pilot did not reveal (e.g. all multiple-choice, all commonsense-reasoning). Remediation would be a task-set redesign, not a re-run of N134.
...
scripts/n134/
  ...
  04_sample_pairs.py         # Phase 3a: commit 45-pair cross-task sample using N134_PAIR_SEED=134
...
sidecar/notes/
  n134_spec.md               # this document
  n134_report.md             # final writeup, created post-Phase 5
```
...
4. **Phase 3a (pair sampling).** Run `04_sample_pairs.py` with `N134_PAIR_SEED=134`. Commits `pair_sample.json` before any merge evaluation runs. This file must be git-committed before Phase 3b begins.
...
**Idempotence requirement.** Every phase must skip its output if the output file exists and is valid (matches the schema version declared in `audit_v2_1.schema.json` for audit, or passes JSON validation for other outputs). This preserves N133's resume-friendly convention.
...
| Gradience | `gradience>=0.11.0` | spectral audit |
...
**Primary target: single RTX 6000 Ada 48GB on RunPod Secure Cloud ($0.74/hr as of April 2026).**
...
3. **A100 80GB PCIe** ($1.19/hr) — ~35% premium for unused VRAM; defensible if the coding agent has existing infrastructure tested against A100 from N133.
...
Scales roughly 3× N133's compute; cost-per-experiment drops ~40% versus N133's H100 path due to platform choice.

### 9.5 Required artifacts in the final report

`sidecar/notes/n134_report.md` must include:
...
- **Whether tasks 1–8 actually land in [0.70, 0.90].** Pilot will tell us. If fewer than 5 tasks land in-band after retries and substitutions, N134 is halted and the task-set design is revised before any Phase 1 training.
- **Whether erank is continuously distributed across the 8 tasks.** Pilot tests via Hartigan's dip on the 8-point distribution. If bimodal, N134 is halted and the task set revised.
...
- **Whether a weight-space score can succeed at decoder scale in principle.** Zhou et al. (2026) found on vision classifiers that weight-space metrics individually achieve |r| < 0.2 with post-merge accuracy while activation-based metrics achieve r > 0.5. Whether this encoder/vision-scale pattern holds at decoder-LLM scale is not known. N134's H1 is a direct test on one specific weight-space score (O-depth alignment) in one specific regime (Mistral-7B, r=16, commonsense-reasoning tasks in [0.70, 0.90] accuracy band). A negative N134 H1 result is consistent with either (i) this particular score being wrong for the regime, or (ii) weight-space pairwise prediction being fundamentally insufficient at decoder scale. Distinguishing (i) from (ii) requires a follow-up study with an activation-informed score variant and is out of scope for N134.
...
3. **§6 expanded** from 3 methods (Gradience / KnOTS / TSV) to 4 methods (+ SVC). Pre-registered direction updated accordingly.
4. **§6 end-notes added** clarifying why TARA-Merging and Core Space Merging are not in the comparison (different intervention class / roadmap for v2.0 product, respectively). OSRM cited as external conceptual reference.
...
No changes to: task set, seeds, training protocol, primary H1 score definition (§4), decision rule thresholds (ρ ≥ 0.50, ΔR² ≥ 0.10), confound constraints C1–C4, statistical protocol (§5), four-method scheduled comparison set (Gradience / KnOTS / TSV / SVC, §6), deviation policy (§7), or execution plan (§9). No new code required.
...
*Prepared by John T. Nanney. Pre-registration commit date: [insert on commit]. Spec v3.1 supersedes v3 in all respects; v1, v2, and v3 are retained in git history for audit.*
