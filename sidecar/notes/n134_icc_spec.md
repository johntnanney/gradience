# N134 Cross-Seed ICC Specification

**Status:** spec, pending implementation.
**Purpose:** close the `\emph{[TODO...]}` in `app:reliability` of
`papers/n134_workshop/draft_v2_thesis_b.tex`.
**Companion artefacts at completion time:**
`sidecar/results/n134/analysis_icc.json`, `scripts/n134/09_analysis_icc.py`,
short Appendix-D paragraph and §2.2 / §4.2 in-text insertions.
...
`scripts/n134/06_analysis_h1.py` applied to a same-task adapter pair. The
estimate is an **instrument-validation** step rather than an exploratory
measurement: it quantifies how much the score for a same-task pair would
shift if one of the two adapters were resampled (i.e., retrained under a
different seed), holding the task fixed.
...
- **Source file.** `sidecar/results/n134/audit/pair_alignment_full.json`.
...
- **Completeness.** 24/24 by design (the N134 pre-registration's
...
- **Single measurement, not average.** The paper's primary H1 test applies
  ...
  three-pair average the N134 design happened to have available for the
  same-task subset.
...
`compute_s_h1` from `scripts/n134/06_analysis_h1.py` directly; do not
...
1. Load `sidecar/results/n134/audit/pair_alignment_full.json`.
...
3. For each pair, compute $S_{\mathrm{H1}}$ by calling
   `compute_s_h1()` imported from `scripts/n134/06_analysis_h1.py`.
...
7. Emit `sidecar/results/n134/analysis_icc.json` per the schema in §5.
...
{
  "instrument": "S_H1",
  "instrument_source": "scripts/n134/06_analysis_h1.py:compute_s_h1",
  "design": {
    "icc_form": "ICC(2,1)",
...
- **Range check.** $\mathrm{ICC} \in [0, 1]$. Negative ICC indicates
  ...
  before reporting.
- **Magnitude expectation.** Prior Gradience program experience with
  spectral scores on seed-replicated adapters in the same task suggests
  ICC > 0.6 is the expected regime. $\mathrm{ICC} < 0.5$ is not a bug
  *per se* but is unexpected enough to warrant a second-look at the panel
  before it is paper-reported.
...
- If the implementer has a principled reason to depart from a design
  ...
  revision note (RN-NNN) and update this spec before proceeding.
...
If a subsequent Gradience study (N135-alt or later) uses the same
procedure, promote §3's design choices to
`sidecar/conventions/cross_seed_icc.md`, following the
`reproducibility_check_tiers.md` pattern (study-specific spec first,
convention abstracted on second use). For the present study this spec is
N134-scoped.
