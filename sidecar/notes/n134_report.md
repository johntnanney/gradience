...
The KnOTS, TSV, and SVC scores are not re-uses of the published reference implementations; they are documented pairwise-triage adaptations of each method's core measurement quantity, implemented directly from the published papers in `scripts/n134/08_compare_methods.py`. The faithfulness and adaptation notes are inline in the score functions' docstrings.
...
**D6 — Per-pair v2.1 audit file shape (Phase 5).** `08_compare_methods.py` was written against a hypothetical per-pair v2.1 directory (`audit/v2.1/*.json`) that was never populated — `03_spectral_audit.py` writes per-adapter `.npz` sidecars instead, and writing the per-adapter v2.1 JSONs in parallel would have exhausted the pod's MooseFS quota. Resolved by writing a lazy per-pair synthesizer (`LazyPairV21` in `08_compare_methods.py`) that reconstructs the per-pair dict shape on demand from the per-adapter SVD factors. No method math changed; only the I/O path differed from the original script's expectation. Fixed in commit `4677dde`.
...
A `pip freeze` snapshot of the pod environment was not captured before the pod was decommissioned. This is a reproducibility limitation: readers attempting a bit-exact replication will need to re-derive a working environment from the library versions above plus the D-series deviation notes. A contemporaneous development-environment `pip freeze` (Python 3.14.x, `torch` 2.9.x, `transformers` 4.57.x) is not substitute-equivalent and is omitted to avoid suggesting false precision.

**Phase 5 method implementations.** KnOTS, TSV, and SVC scores are *not* re-uses of the published reference implementations. Each of the three methods was re-implemented directly from the source paper as a pairwise-triage adaptation of that method's core measurement quantity. The adaptations are inline in `scripts/n134/08_compare_methods.py` — specifically `knots_score_from_v21`, `tsv_score_from_v21`, and `svc_score_from_v21`, each with a docstring that documents both the faithfulness (what quantity is being measured) and the departure (what portfolio-scale pipeline is *not* being executed). The paper citations are in the bibliography (`papers/n134_workshop/references.bib`); pinned commits for the published KnOTS / TSV / SVC repositories are not applicable because those repositories were not used as dependencies.
