...
**Verify the write before moving on.** After writing the report, re-read the destination path and confirm the first ~5 lines match what you intended (date header, agent invocation timestamp, the first sentence of the orientation summary). If they do not match — for example, if a separate agent run on the same day overwrote your write between your write and your verify, or if an osascript-mediated copy from the agent-side workspace silently no-op'd — you have two recovery options: (a) write to a separate file with a disambiguating suffix (`<YYYY-MM-DD>-second-pass.md`), preserving whatever else landed there; or (b) re-write the file with the `## Second pass at HH:MM UTC` header appended to the existing content. Do not declare the report committed until a `diff -q` between source and destination (or a head/tail spot-check) shows the destination contains your content. This step exists because the osascript-mediated copy path from the agent-side filesystem to the gradience repo has historically failed silently in race conditions with concurrent same-day runs (see 2026-05-03 for the case that prompted this discipline); verify-after-write is the simplest catch.
