    ...
    def test_udr_handles_base_norm_near_zero(self):
        ...
        for base_sigma in small_base_norms:
            for base_fro in small_base_norms:
                ...
                udr, udr_f, sdi = compute_udr_metrics(delta_sigma_max, delta_fro_norm, base_sigma, base_fro, eps=eps)
                ...
                assert abs(udr - expected_udr) < 1e-10, f"UDR mismatch: expected {expected_udr}, got {udr}"
...
class TestCacheRobustness:
    ...
    def test_cache_write_read_roundtrip(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Verify exact match
            assert loaded_norms == test_norms, f"Cache roundtrip failed: expected {test_norms}, got {loaded_norms}"
    ...
    def test_corrupt_cache_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            loaded_norms = load_base_model_norms(base_norms_cache=cache_path, issues=issues)
    ...
    def test_missing_cache_file(self):
        ...
        loaded_norms = load_base_model_norms(base_norms_cache=nonexistent_path, issues=issues)
