...
class TestLoRANormComputation:
    ...
    def generate_random_lora_factors(
        self, out_dim: int, in_dim: int, rank: int, scaling: float = 1.0
    ...
    def compute_explicit_delta_w_norms(self, A: torch.Tensor, B: torch.Tensor, scaling: float) -> tuple[float, float]:
    ...
    def test_small_random_matrices_accuracy(self, random_seed):
        ...
        for out_dim, in_dim, rank, scaling in test_cases:
            ...
            fro_efficient, spectral_efficient = compute_lora_norms(
                A, B, scaling, compute_dtype=torch.float64, device="cpu"
            )
    ...
    def test_scaling_correctness(self, random_seed):
        ...
        assert abs(spectral_2 - 2 * spectral_1) < 1e-10, (
            f"Spectral norm scaling incorrect: {spectral_2} vs 2*{spectral_1}"
        )
    ...
    def test_near_zero_eigenvalues_stability(self, random_seed):
        ...
        assert abs(spectral_norm - spectral_explicit) < 1e-8, (
            "Should match explicit computation even with small eigenvalues"
        )
...
class TestSqrtPsdUtility:
    ...
    def test_sqrt_psd_with_zero_eigenvalues(self):
        ...
        assert frobenius_error < 1e-10, (
            f"Reconstruction error {frobenius_error:.2e} too large even for rank-deficient case"
        )
...
class TestIntegrationEndToEnd:
    ...
    def test_complete_workflow_small_example(self):
        ...
        modules_and_factors = [
            (
                "model.layers.0.attn.q",
                torch.randn(2, 8, dtype=torch.float64),
                torch.randn(12, 2, dtype=torch.float64),
                1.0,
            ),
            (
                "model.layers.0.attn.k",
                torch.randn(2, 8, dtype=torch.float64),
                torch.randn(12, 2, dtype=torch.float64),
                1.0,
            ),
            (
                "model.layers.1.attn.q",
                torch.randn(2, 8, dtype=torch.float64),
                torch.randn(12, 2, dtype=torch.float64),
                2.0,
            ),  # Higher scaling
            (
                "model.layers.2.attn.q",
                torch.randn(2, 8, dtype=torch.float64),
                torch.randn(12, 2, dtype=torch.float64),
                0.5,
            ),  # Lower scaling
        ...
        print(
            f"Integration test passed: {len(modules_and_factors)} modules, "
