...
class TestGuardCPUValidation(unittest.TestCase):
    ...
    def test_1_grad_explosion_triggers_rollback(self):
        ...
        # Create driver
        driver = HFCallbackDriver(callback_class=GradienceCallback, callback_config=guard_config)
        ...
        try:
            ...
            self.assertEqual(trigger_type, "grad_explosion", f"Expected trigger 'grad_explosion', got '{trigger_type}'")
            print("   ✓ GUARD_TRIGGERED found with trigger=grad_explosion")
            ...
            print("   ✓ Found metrics with action=rollback")
            ...
            print("=" * 60)
            print("Summary:")
            print("  • Guard detected grad_norm=1e6 > threshold=100.0")
            print("  • Triggered grad_explosion at step 2")
            print(f"  • Rolled back to step {restored_step}")
            print("  • All required telemetry events present")
    ...
    def test_2_cooldown_lockout_aborts(self):
        ...
        try:
            ...
            self.assertEqual(len(rollback_alerts), 1, f"Expected 1 GUARD_ROLLBACK, got {len(rollback_alerts)}")
            ...
            print("   ✓ Found metrics with action=abort")
            ...
            print("=" * 60)
            print("Summary:")
            print("  • First NaN at step 10 triggered rollback")
            print("  • Second NaN at step 15 aborted due to cooldown protection")
            print("  • Cooldown window prevents rollbacks within 20 steps")
            print("  • Anti-thrash protection working correctly")
            print(f"  • JSONL file: {result.telemetry_file}")
    ...
    def test_3_window_max_rollbacks_aborts(self):
        ...
        try:
            ...
            self.assertEqual(len(triggered_alerts), 2, f"Expected 2 GUARD_TRIGGERED, got {len(triggered_alerts)}")

            trigger_steps = result.get_alert_steps("GUARD_TRIGGERED")
            self.assertEqual(set(trigger_steps), {10, 11}, f"Expected triggers at steps 10,11, got {trigger_steps}")
            ...
            print("=" * 60)
            print("Summary:")
            print("  • First trigger at step 10 successfully rolled back")
            print("  • Second trigger at step 11 aborted (max_rollbacks=1 in window)")
            print("  • Window-based anti-thrash protection working correctly")
            print("  • No cooldown interference (cooldown_steps=0)")
            print(f"  • JSONL file: {result.telemetry_file}")
