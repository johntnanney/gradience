...
class TestSafeUniformPolicyRegression(unittest.TestCase):
    ...
    def test_policy_file_exists_and_loads(self):
        ...
        self.assertTrue(self.policy_path.exists(), f"Safe uniform policy file missing: {self.policy_path}")
    ...
    def test_policy_version_and_metadata(self):
        ...
        self.assertEqual(self.policy["policy_version"], "0.1", "Policy version changed - check if this is intentional")
    ...
    def test_safety_criteria_thresholds(self):
        ...
        self.assertEqual(criteria["pass_rate_min"], 0.67, "Pass rate threshold changed - this invalidates calibration")
        self.assertEqual(
            criteria["worst_delta_min"], -0.025, "Worst delta threshold changed - this invalidates calibration"
        )
    ...
    def test_primary_recommendation_r20(self):
        ...
        self.assertEqual(
            recommendations["primary_uniform_rank"],
            20,
            "Primary uniform rank changed from r=20 - calibration invalidated",
        )
        self.assertEqual(
            recommendations["conservative_uniform_rank"], 24, "Conservative uniform rank changed from r=24"
        )
        self.assertIn(
            16,
            recommendations["avoid_uniform_ranks"],
    ...
    def test_empirical_evidence_integrity(self):
        ...
        self.assertEqual(
            r16_evidence["pass_rate"], 0.0, "r=16 pass rate evidence changed - this was empirically unsafe"
        )
        self.assertEqual(r16_evidence["worst_delta"], -0.080, "r=16 worst delta evidence changed")
        self.assertEqual(r16_evidence["status"], "RISKY", "r=16 safety status changed - contradicts empirical evidence")

        # r=20: safe (-0.008 worst delta, 100% pass rate)
        r20_evidence = evidence["uniform_r20"]
        self.assertEqual(r20_evidence["pass_rate"], 1.0, "r=20 pass rate evidence changed")
        self.assertEqual(r20_evidence["worst_delta"], -0.008, "r=20 worst delta evidence changed")
        self.assertEqual(r20_evidence["status"], "SAFE", "r=20 safety status changed - contradicts empirical evidence")
    ...
    def test_fallback_strategy_documented(self):
        ...
        self.assertEqual(
            fallback["default_recommendation"],
            "per_layer_adaptive",
            "Fallback strategy changed from per-layer adaptive",
        )
        self.assertIn("No policy-compliant uniform compression found", fallback["fallback_reason"])
    ...
    def test_prose_documentation_consistency(self):
        ...
        self.assertIn("Uniform r=20", self.validation_policy_text, "r=20 recommendation missing from validation policy")
        self.assertIn(
            "Uniform r=16", self.validation_policy_text, "r=16 safety information missing from validation policy"
        )

        # Bench README should contain current baseline
        self.assertIn("**r=20**", self.bench_readme_text, "r=20 primary recommendation missing from Bench README")
        self.assertIn(
            "TASK/MODEL DEPENDENT", self.bench_readme_text, "Task/model dependency warning missing from Bench README"
        )
    ...
    def test_policy_limitations_documented(self):
        ...
        self.assertIn("Task/model specific", limitation_text, "Task/model limitation warning missing")
        self.assertIn("Re-validate", limitation_text, "Re-validation requirement missing")
        self.assertIn("SST-2", limitation_text, "Dataset-specific limitation missing")
