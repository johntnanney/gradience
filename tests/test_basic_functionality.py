...
class TestBasicFunctionality:
    ...
    def test_cli_help_commands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "gradience.cli", "--help"], capture_output=True, text=True, timeout=30
        )
        ...
        # Test bench CLI
        result = subprocess.run(
            [sys.executable, "-m", "gradience.bench.run_bench", "--help"], capture_output=True, text=True, timeout=30
        )
        assert result.returncode == 0
...
class TestConfigValidation:
    ...
    def test_all_configs_parse(self):
        ...
        if errors:
            pytest.fail("Config parsing errors:\n" + "\n".join(errors))
