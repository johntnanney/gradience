...
class TestHFCallbackGuardWiring(unittest.TestCase):
    ...
    def test_guard_wiring_basic_functionality(self):
        ...
        try:
            callback.on_log(mock_args, mock_state, mock_control, logs={"loss": 1.0, "grad_norm": 5.0}, model=model)
        except Exception as e:
        ...
        with open(telemetry_file) as f:
            for line in f:
                if line.strip():
                    ...
                    elif event.get("event") == "metrics" and event.get("kind") == "guard":
        ...
        self.assertGreater(len(guard_alerts), 0, "Should have at least one Guard alert (TRIGGERED, ROLLBACK, or ABORT)")

        self.assertGreater(len(guard_metrics), 0, "Should have at least one Guard metric (init, snapshot, or rollback)")
        ...
        self.assertTrue(has_expected, f"Should have canonical Guard alerts, got: {actual_codes}")
    ...
    def test_guard_disabled_by_default(self):
        ...
        try:
            ...
            callback.on_log(mock_args, mock_state, mock_control, logs={"loss": float("nan")}, model=Mock())
            callback.on_train_end(mock_args, mock_state, mock_control)
        except Exception as e:
    ...
    def test_guard_with_mock_model_parameters(self):
        ...
        mock_model.named_parameters.return_value = [
            (
                ...
                Mock(
                    detach=Mock(
                        return_value=Mock(to=Mock(return_value=Mock(clone=Mock(return_value=torch.randn(4, 8)))))
                    )
                ),
            )
        ]
    ...
    def test_anti_thrash_abort_behavior(self):
        ...
        callback.on_log(mock_args, mock_state, mock_control, logs={"loss": 1.0}, model=model)

        # First NaN trigger - should trigger rollback
        mock_state.global_step = 10
        callback.on_log(mock_args, mock_state, mock_control, logs={"loss": float("nan")}, model=model)

        # Verify first rollback succeeded and training continues
        self.assertFalse(mock_control.should_training_stop, "Training should continue after first rollback")
        ...
        self.assertTrue(mock_control.should_training_stop, "Training should stop after exceeding max_rollbacks")
        ...
        self.assertEqual(guard_rollback_count, 1, "Should have exactly 1 GUARD_ROLLBACK before abort")
        self.assertTrue(guard_abort_found, "Should have GUARD_ABORT when max_rollbacks exceeded")

        print("✓ Anti-thrash test passed: 1 rollback, then abort with training stop")
    ...
    def test_enable_guard_false_does_nothing(self):
        ...
        try:
            ...
            for step in [1, 2, 3, 4, 5]:
                ...
                callback.on_log(
                    mock_args, mock_state, mock_control, logs={"loss": 1.0 + step * 0.1, "grad_norm": 5.0}, model=model
                )

            # 3. Problematic inputs that would trigger Guard if enabled
            mock_state.global_step = 10
            callback.on_log(
                mock_args, mock_state, mock_control, logs={"loss": float("nan"), "grad_norm": float("inf")}, model=model
            )

            mock_state.global_step = 15
            callback.on_log(
                mock_args, mock_state, mock_control, logs={"loss": float("inf"), "grad_norm": 1e9}, model=model
            )

            # 4. More normal training
            mock_state.global_step = 20
            callback.on_log(mock_args, mock_state, mock_control, logs={"loss": 2.0, "grad_norm": 3.0}, model=model)

            # 5. Evaluation
            callback.on_evaluate(mock_args, mock_state, mock_control, metrics={"eval_loss": 1.5, "eval_accuracy": 0.8})
        ...
        self.assertFalse(mock_control.should_training_stop, "Training should never be stopped when Guard is disabled")
        ...
        self.assertEqual(len(guard_events), 0, f"Should have NO Guard events when disabled, found: {guard_events}")
        self.assertGreater(len(train_events), 0, "Should still log normal training events")
        self.assertGreater(len(eval_events), 0, "Should still log evaluation events")

        print("✓ Guard disabled safety invariant verified:")
        print("  • 0 Guard events (expected: 0)")
        print(f"  • {len(train_events)} train events (expected: >0)")
        print(f"  • {len(eval_events)} eval events (expected: >0)")
        print("  • No training interference with NaN/Inf inputs")
