...
class LogEvent:
    ...
    additional_logs: dict[str, Any] | None = None
...
class HFCallbackDriver:
    ...
    def run_scenario(self, scenario: GuardScenario) -> "CallbackResult":
        ...
        """
        return self.run_events(log_events=scenario.log_events, scenario_name=scenario.name)

    def run_events(self, log_events: list[LogEvent], scenario_name: str = "test") -> "CallbackResult":
        """
        ...
        self.callback.on_train_begin(self.mock_args, self.mock_state, self.mock_control, model=self.model)
        ...
        for log_event in log_events:
            ...
            self.callback.on_log(self.mock_args, self.mock_state, self.mock_control, logs=logs, model=self.model)

        # 3. End training
        self.callback.on_train_end(self.mock_args, self.mock_state, self.mock_control)
        ...
        return CallbackResult(
            events=self.events, telemetry_file=self.telemetry_file, callback=self.callback, scenario_name=scenario_name
        )
...
class CallbackResult:
    ...
    def __init__(self, events: list[dict[str, Any]], telemetry_file: Path, callback: Any, scenario_name: str):
        self.events = events
    ...
    def assert_alert_present(self, alert_code: str, message: str = None) -> dict[str, Any]:
        ...
        assert matching, (
            f"Alert '{alert_code}' not found in telemetry. Available: {[e.get('code') for e in self.guard_alerts]}"
        )
    ...
    def assert_rollback_count(self, expected_count: int):
        ...
        if hasattr(self.callback, "guard") and self.callback.guard:
            ...
            assert len(rollback_alerts) == expected_count, (
                f"Expected {expected_count} rollback alerts, got {len(rollback_alerts)}"
            )
...
def create_grad_explosion_scenario(
    trigger_step: int = 10, grad_norm: float = 500.0, guard_config_overrides: dict[str, Any] | None = None
) -> GuardScenario:
...
def create_cooldown_scenario(first_step: int = 10, second_step: int = 15, cooldown_steps: int = 20) -> GuardScenario:
...
def create_max_rollbacks_scenario(first_step: int = 10, second_step: int = 11, max_rollbacks: int = 1) -> GuardScenario:
