...
def create_tiny_lora_adapter(model_name: str, out_dir: Path) -> bool:
    ...
    try:
        ...
        tiny_data = ["The quick brown fox", "Hello world example", "Test training data"]
...
class TestUDRIntegration:
    ...
    def test_end_to_end_audit_emits_udr_keys(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            ).exists(), "Missing adapter weights"

            # Test audit WITHOUT base model (should have no UDR)
            print("Testing audit without base model...")
            cmd = ["gradience", "audit", "--peft-dir", str(adapter_dir), "--json"]

            result = subprocess.run(cmd, capture_output=True, text=True, env=get_test_env())
            ...
            cmd_with_base = ["gradience", "audit", "--peft-dir", str(adapter_dir), "--base-model", model_name, "--json"]

            result_with_base = subprocess.run(cmd_with_base, capture_output=True, text=True, env=get_test_env())
            ...
            print("✅ UDR integration test passed!")
...
class TestCLIBehavior:
    ...
    def test_no_udr_flag_disables_udr(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config = {"peft_type": "LORA", "r": 4, "lora_alpha": 8, "target_modules": ["dense"], "task_type": "SEQ_CLS"}
            ...
            base_norms = {"model.layer.0.dense": {"sigma_max": 10.0, "fro_norm": 50.0}}
...
if __name__ == "__main__":
    ...
    test_classes = [
        ...
        (
            cli_tests,
            [
                "test_no_udr_flag_disables_udr",
                "test_base_model_flag_enables_udr",
                "test_bad_base_model_gives_useful_error",
            ],
        ),
    ]
