...
class TestPerLayerSuggestionsInvariants(unittest.TestCase):
    ...
    def test_suggested_r_positive(self):
        ...
        for suggestion in rep.layers:
            with self.subTest(layer=suggestion.name):
                self.assertGreater(
                    suggestion.suggested_r, 0, f"suggested_r should be positive, got {suggestion.suggested_r}"
                )
        ...
        for suggestion in rep.layers:
            with self.subTest(layer=suggestion.name):
                self.assertGreaterEqual(
                    suggestion.reduction_ratio, 0.0, f"reduction_ratio should be >= 0, got {suggestion.reduction_ratio}"
                )
                self.assertLessEqual(
                    suggestion.reduction_ratio, 1.0, f"reduction_ratio should be <= 1, got {suggestion.reduction_ratio}"
                )
    ...
    def test_handles_missing_stable_rank(self):
        ...
        try:
            ...
            layer_with_missing = next((s for s in rep.layers if s.name == "layer2"), None)
    ...
    def test_new_layer_data_structure_compatibility(self):
        ...
        new_structure_audit = {"layer_data": {"layer_rows_schema": "v1", "layer_rows": self.basic_audit["layers"]}}
