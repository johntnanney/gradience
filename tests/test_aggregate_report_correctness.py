...
class TestAggregateReportCorrectness(unittest.TestCase):
            ...
            report = {
                ...
                "env": {
                    ...
                    "validation_classification": {
                        "level": "screening",
                        "rationale": f"Single seed, 1000 steps (no variance estimation)",
                        "is_multiseed": False,
                        "n_seeds": 1,
                        "max_steps": 1000
                    }
                ...
                "summary": {
                    "recommendations_validated": "2/2",
                    "best_compression": "uniform_r32"
                },
                "config_metadata": {
                    "primary_metric_key": "eval_exact_match",
                    "embedded_config": {
                        "train": {"max_steps": 1000, "seed": seed}
                    }
                }
            }
    ...
    def test_validation_classification_multiseed_fix(self):
        ...
        assert validation_classification["n_seeds"] == 3, f"Should show 3 seeds, got {validation_classification['n_seeds']}"
        assert "certifiable" in validation_classification["level"], f"Expected certifiable level with 3 seeds x 1000 steps, got {validation_classification['level']}"
        assert "statistical rigor" in validation_classification["rationale"], "Rationale should mention statistical rigor"
    ...
    def test_verdict_logic_stringent_threshold_fix(self):
        ...
        assert result_mixed["compressed"]["energy_p90"]["verdict"] == "FAIL", "Should fail with 66.7% pass rate (below 80% threshold)"
        assert result_mixed["compressed"]["uniform_r32"]["verdict"] == "FAIL", "Should fail with 66.7% pass rate (below 80% threshold)"
    ...
    def test_overall_verdict_policy_compliance(self):
        ...
        for passes, expected_rate, expected_verdict in test_cases:
            ...
            assert actual_verdict == expected_verdict, f"Expected verdict {expected_verdict}, got {actual_verdict} for {expected_rate*100}% pass rate"
