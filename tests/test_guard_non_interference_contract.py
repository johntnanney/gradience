...
class TestGuardNonInterferenceContract(unittest.TestCase):
    ...
    def test_guard_disabled_has_zero_interference(self):
        ...
        driver = HFCallbackDriver(callback_class=GradienceCallback, callback_config=config)
        ...
        try:
            ...
            self.assertIsNone(driver.callback.guard, "Guard object should NOT be created when disabled")
            ...
            guard_alerts = [
                e for e in result.events if e.get("event") == "alert" and e.get("code", "").startswith("GUARD_")
            ]
            self.assertEqual(len(guard_alerts), 0, f"Found {len(guard_alerts)} Guard alerts when disabled!")
            print(f"   ✅ No Guard alerts emitted (checked {len(result.events)} events)")

            # Check for any Guard metrics
            guard_metrics = [e for e in result.events if e.get("event") == "metrics" and e.get("kind") == "guard"]
            self.assertEqual(len(guard_metrics), 0, f"Found {len(guard_metrics)} Guard metrics when disabled!")
            ...
            self.assertEqual(len(guard_related), 0, f"Found {len(guard_related)} events mentioning 'guard'!")
            ...
            self.assertNotEqual(should_stop, True, "Training was stopped despite Guard being disabled!")
            ...
            self.assertIsNone(driver.callback.guard, "Guard should be None, so no snapshots possible")
    ...
    def test_guard_disabled_with_mock_trainer_state_mutations(self):
        ...
        try:
            ...
            print("\n1. Model state check...")
            self.assertEqual(
                initial_model_state, final_model_state, "Model state was mutated despite Guard being disabled!"
            )
            print("   ✅ Model state unchanged")

            print("\n2. Training arguments check...")
            self.assertEqual(initial_args, final_args, "Training args were mutated despite Guard being disabled!")
    ...
    def test_guard_disabled_is_default_safe(self):
        ...
        self.assertFalse(default_config.enable_guard, "Guard must be disabled by default for safety!")
        ...
        self.assertIsNone(callback.guard, "Guard should not be created with default config")
