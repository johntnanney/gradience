...
class TestUDROptInPolicy:
    ...
    def create_minimal_config(self, audit_config=None):
        ...
        config = {
            ...
            "compression": {"allowed_ranks": [2, 4, 8, 16], "acc_tolerance": 0.025},
            "runtime": {"device": "cpu"},
            "bench_version": "0.1",
            "run_type": "test",
        }
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                mock_audit.assert_called_with(
                    str(probe_dir), base_model_id=None, base_norms_cache=None, compute_udr=False
                )
    ...
    def test_proper_udr_opt_in_succeeds(self, mock_audit):
        ...
        config = self.create_minimal_config({"compute_udr": True, "base_model": "distilbert-base-uncased"})
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            mock_audit.assert_called_with(
                str(probe_dir), base_model_id="distilbert-base-uncased", base_norms_cache=None, compute_udr=True
            )

    def test_udr_opt_in_with_base_norms_cache(self):
        """Test UDR opt-in with base_norms_cache option."""
        config = self.create_minimal_config(
            {"compute_udr": True, "base_model": "distilbert-base-uncased", "base_norms_cache": "/path/to/cache"}
        )
