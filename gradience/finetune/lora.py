...
class LoRAAnalyzer:
    ...
    def _param_to_adapter_name(self, param_name: str) -> str | None:
    ...
    def analyze_structural(
        ...
        layers_dominating = [name for name, d in per_layer_scaled.items() if d > self.DOMINANCE_WARNING]
    ...
    def suggest_rank(self, safety_margin: float = 1.5) -> dict[str, Any]:
        ...
        if recommended < current_rank:
            ...
            reasoning = (
                f"Your adapters are fully utilizing rank-{current_rank}. Current configuration appears appropriate."
            )
    ...
    def report(self, lora_alpha: float | None = None, lora_rank: int | None = None) -> str:
        ...
        if lora_alpha is not None and lora_rank is not None:
            ...
            lines.append("  (1.0 = perfect, <0.001 = problematic)")
            lines.append("")
