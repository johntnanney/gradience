...
class PolicyScoreboard:
    ...
    def __init__(self, scoreboard_path: Path | None = None):
    ...
    def add_benchmark_results(
        self, config_name: str, model_name: str, task_name: str, policy_results: list[PolicyBenchmarkResult]
    ) -> None:
        ...
        if result.best_candidate_delta < 0:
            ...
            if result.performance_delta >= result.best_candidate_delta:
                ...
                return max(
                    0.0,
                    1.0
                    - abs(result.performance_delta - result.best_candidate_delta) / abs(result.best_candidate_delta),
                )
        else:
    ...
    def _recalculate_metrics(self) -> None:
        ...
        for policy_name, policy_data in self.data["policies"].items():
            ...
            policy_data["avg_optimality"] = (
                statistics.mean(policy_data["optimality_scores"]) if policy_data["optimality_scores"] else 0.0
            )
    ...
    def _calculate_trend(self, performance_history: list[float]) -> str:
        ...
        if len(performance_history) >= 10:
            ...
            previous = (
                ...
                else performance_history[: len(performance_history) // 2]
            )
    ...
    def get_policy_metrics(self, policy_name: str) -> PolicyMetrics | None:
    ...
    def get_best_policies(self, top_n: int = 5) -> list[tuple[str, PolicyMetrics]]:
        ...
        for policy_name in self.data["policies"]:
            ...
            if metrics and metrics.total_attempts >= 3:  # Minimum attempts for ranking
                ...
                score = metrics.avg_optimality * 0.5 + metrics.pass_rate * 0.3 + metrics.reliability_score * 0.2
...
def create_policy_result_from_bench_data(
    config_name: str,
    model_name: str,
    task_name: str,
    policy_name: str,
