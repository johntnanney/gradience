...
from .audit.lora_audit import LoRAAdapterConfig, find_peft_files, load_adapter_state_dict, load_peft_adapter_config
...
class SVDTruncationReport:
    ...
    per_module_energy: list[dict[str, str | int | float]]  # Per-module details
    alpha_mode: str
    ...
    total_original_params: int | None = None
    total_new_params: int | None = None
...
def _matches_lora_A(key: str, adapter_name: str = "default") -> bool:
    ...
    return f".lora_A.{adapter_name}.weight" in key or (key.endswith(".lora_A.weight") and adapter_name == "default")
    ...
    for key, tensor in state_dict.items():
        if _matches_lora_A(key, adapter_name):
            try:
                ...
                complete_pairs[base_key] = {"A": (key, lora_A), "B": (lora_B_key, lora_B)}
...
def _update_adapter_config(
    config: LoRAAdapterConfig, target_rank: int, alpha_mode: Literal["keep_ratio", "keep_alpha"] = "keep_ratio"
) -> dict:
    """
...
def svd_truncate_peft_dir(
    ...
    *,
    ...
    # Process each pair
    new_state_dict = {}
    per_module_energy: list[dict[str, str | int | float]] = []
    ...
    if not readme_path.exists():  # Don't overwrite existing README
        with open(readme_path, "w") as f:
            f.write(_generate_truncation_readme(truncation_report, peft_dir, copied_files, weights_out_path.name))

    return truncation_report


def _generate_truncation_readme(
    report: SVDTruncationReport, source_dir: Path, copied_files: list[str], weights_filename: str
) -> str:
