...
class TrainingMonitor:
    ...
    def update(self, step: int, metrics: Dict[str, Any]) -> List[MonitorAlert]:
        ...
        if len(active_plateaus) >= 2:
            a = self._emit_if_cool(step, MonitorAlert(
                step=step,
                code="CONSIDER_STOPPING",
                severity="warning",
                message=(
                    "Both loss and gradient norm have plateaued. "
                    "Training dynamics appear frozen."
                ),
    ...
    def _check_loss_plateau(self, step: int) -> Optional[MonitorAlert]:
        ...
        if self._loss_stale_count >= self.config.loss_plateau_patience:
            return self._emit_if_cool(step, MonitorAlert(
                step=step,
                code="LOSS_PLATEAU",
                severity="info",
                message=(
    ...
    def _check_grad_plateau(self, step: int) -> Optional[MonitorAlert]:
    ...
    def _check_grad_spike(self, step: int, grad_norm: float) -> Optional[MonitorAlert]:
        """Check for gradient norm spikes."""
        if len(self._grad_norm_history) < 5:
            return None

        # Running mean of recent grad norms (excluding current)
        history = list(self._grad_norm_history)[:-1] if len(self._grad_norm_history) > 1 else list(self._grad_norm_history)
        ...
        if ratio > self.config.grad_spike_multiplier:
            return self._emit_if_cool(step, MonitorAlert(
                ...
                severity="warning",
                message=(
                    f"Gradient norm spike: {grad_norm:.4f} "
                    f"({ratio:.1f}x the running mean of {running_mean:.4f})"
                ),
                details={
                    "grad_norm": grad_norm,
    ...
    def _check_eval_plateau(self, step: int) -> Optional[MonitorAlert]:
    ...
    def _check_utilization(self, step: int, utilization: float) -> Optional[MonitorAlert]:
    ...
    def _emit_if_cool(self, step: int, alert: MonitorAlert) -> Optional[MonitorAlert]:
