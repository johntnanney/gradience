...
def compute_lora_norms(
    ...
    *,
    ...
    singular_values = low_rank_singular_values(A_compute, B_compute, compute_dtype=compute_dtype, eps=eps)
...
def compute_lora_stable_rank(
    ...
    *,
    ...
    frobenius_norm, spectral_norm = compute_lora_norms(
        A, B, scaling, compute_dtype=compute_dtype, device=device, eps=eps
    )
...
def compute_layer_energy_concentration(
    module_energies: dict[str, float], *, top_k: int = 5, eps: float = 1e-12
) -> dict:
    ...
    for layer_idx in sorted(layer_energies.keys()):
        ...
        layers_list.append({"layer": layer_idx, "energy_fro2": energy, "share": share})
...
def compute_lora_energy_concentration(
    layer_energies: torch.Tensor, *, top_k_fraction: float = 0.1, eps: float = 1e-12
) -> tuple[float, float]:
