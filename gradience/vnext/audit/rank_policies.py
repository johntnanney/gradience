...
class RankSuggestion:
    ...
    details: dict[str, float | int | str | None]  # Policy-specific metadata
...
def apply_rank_policy(
    policy_spec: RankPolicySpec, s: np.ndarray, shape: tuple[int, int], r_alloc: int, eps: float = 1e-12
) -> RankSuggestion:
    """
...
def _energy_threshold_policy(
    s: np.ndarray, shape: tuple[int, int], r_alloc: int, params: dict[str, float | int | str], eps: float
) -> RankSuggestion:
    ...
    if len(s) == 0 or np.sum(s > eps) == 0:
        return RankSuggestion(
            k=0, confidence=0.0, details={"threshold": threshold, "reason": "no_significant_singular_values"}
        )
    ...
    if total_energy <= eps:
        return RankSuggestion(k=0, confidence=0.0, details={"threshold": threshold, "total_energy": total_energy})
...
def _entropy_effective_policy(
    ...
    if len(s) == 0:
        return RankSuggestion(k=0, confidence=0.0, details={"reason": "empty_singular_values"})

    s_clean = s[s > eps]
    if len(s_clean) == 0:
        return RankSuggestion(k=0, confidence=0.0, details={"reason": "no_significant_singular_values"})
...
def _optimal_hard_threshold_policy(
    ...
    if median_sv <= eps:
        return RankSuggestion(
            ...
            confidence=0.0,
            details={"reason": "median_sv_zero", "beta": beta, "omega": omega, "tau": 0.0, "median_sv": median_sv},
        )
...
def _knee_elbow_policy(
    ...
    if total_energy <= eps:
        return RankSuggestion(k=1, confidence=0.0, details={"reason": "negligible_total_energy"})
...
def analyze_policy_consensus(
    policies: list[RankPolicySpec], s: np.ndarray, shape: tuple[int, int], r_alloc: int, eps: float = 1e-12
) -> PolicyConsensus:
    """
    ...
    if not suggestions:
        return PolicyConsensus(
            suggestions=[], median_k=0, k_range=(0, 0), high_confidence_policies=[], disagreement_score=float("inf")
        )
    ...
    high_conf = [policy_spec.name for policy_spec, sugg in zip(policies, suggestions) if sugg.confidence > 0.8]
