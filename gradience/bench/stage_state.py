...
class StageStateManager:
    ...
    def mark_variant_completed(self, variant_name: str, metadata: Optional[Dict[str, Any]] = None):
        """
    ...
    def get_completed_stages(self) -> Set[str]:
        ...
        return {
            name for name, info in self._state["stages"].items()
    ...
    def get_completed_variants(self) -> Set[str]:
        ...
        return {
            name for name, info in self._state["variants"].items()
            if info.get("completed", False)
        }
    ...
    def clean_invalid_state(self):
        ...
        if cleaned_stages or cleaned_variants:
            self._save_state()
            print(f"🧹 Cleaned invalid state entries:")
