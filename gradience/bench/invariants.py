...
class InvariantChecker:
    ...
    def check_rank_heterogeneity(self, rank_pattern: dict[str, int], variant_name: str = "per_layer") -> dict[str, Any]:
        ...
        if not passed and variant_name == "per_layer":
            self.failures.append(
                f"Rank heterogeneity failed for {variant_name}: only {len(unique_ranks)} distinct ranks"
            )

        self.invariants["rank_heterogeneity"] = result
        return result

    def check_layer_consistency(self, rank_pattern: dict[str, int], alpha_pattern: dict[str, int]) -> dict[str, Any]:
        """Check if rank and alpha patterns have matching layer keys."""
        result: dict[str, Any]
        if not rank_pattern or not alpha_pattern:
            result = {"name": "layer_consistency", "status": "SKIPPED", "message": "Missing rank or alpha pattern"}
        ...
        if not passed:
            ...
            self.failures.append("Layer consistency failed: rank/alpha pattern keys don't match")

        self.invariants["layer_consistency"] = result
        return result

    def check_param_counting(
        self, compression_configs: dict[str, Any], probe_audit: dict | None = None
    ) -> dict[str, Any]:
            ...
            if verdict.get("status") == "evaluated":
                ...
                if verdict.get("compressed_accuracy") is not None and verdict.get("probe_accuracy") is not None:
                    ...
                    if abs(expected_delta - reported_delta) > 0.001:  # Tolerance for float precision
                        issue = (
                            f"{variant}: Delta mismatch (expected={expected_delta:.4f}, reported={reported_delta:.4f})"
                        )
                        issues.append(issue)
