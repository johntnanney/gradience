...
def get_git_commit() -> str | None:
    ...
    try:
        result = subprocess.run(
            ["git", "rev-parse", "HEAD"], capture_output=True, text=True, timeout=GIT_SUBPROCESS_TIMEOUT
        )
...
def get_git_tag() -> str | None:
    ...
    try:
        ...
        diff_result = subprocess.run(["git", "diff", "--quiet"], capture_output=True, timeout=GIT_SUBPROCESS_TIMEOUT)
        if diff_result.returncode != 0:
            return "dirty"

        # Check if there are staged changes
        cached_result = subprocess.run(
            ["git", "diff", "--cached", "--quiet"], capture_output=True, timeout=GIT_SUBPROCESS_TIMEOUT
        )
        ...
        describe_result = subprocess.run(
            ["git", "describe", "--tags", "--abbrev=7"], capture_output=True, text=True, timeout=GIT_SUBPROCESS_TIMEOUT
        )
...
def get_hf_model_revision(model_id: str) -> dict[str, str | None] | None:
    ...
    try:
        ...
        return {"model_id": model_id, "revision": "unknown", "last_modified": None}


def get_dataset_revision(dataset_id: str, split: str | None = None) -> dict[str, Any] | None:
