...
def normalize_rank_pattern(rank_pattern: dict) -> dict:
    ...
    """
    return {normalize_peft_module_name(name): rank for name, rank in rank_pattern.items()}
...
def normalize_alpha_pattern(alpha_pattern: dict) -> dict:
    ...
    Returns:
        Dictionary with normalized module names
    """
    return {normalize_peft_module_name(name): alpha for name, alpha in alpha_pattern.items()}


def create_complete_rank_pattern(partial_rank_pattern: dict, audit_layers: list, default_rank: int) -> dict:
    """
...
def create_complete_alpha_pattern(partial_alpha_pattern: dict, audit_layers: list, default_alpha: int) -> dict:
    """
...
def find_adapter_weights_path(variant_dir) -> str:
    ...
    checkpoint_dirs = sorted(
        [d for d in variant_path.glob("checkpoint-*") if d.is_dir()], key=lambda p: os.path.getmtime(p), reverse=True
    )
    ...
    raise FileNotFoundError(
        f"No adapter_model.safetensors found in {variant_dir} using search order: "
