...
def test_policy_integration():
    ...
    for name, policies in test_cases:
        ...
        # Run audit
        try:
            result = audit_lora_state_dict(state_dict, rank_policies=policies, include_top_singular_values=5)
            ...
            if result.layers:
                ...
                if layer.rank_suggestions:
                    ...
                    for policy, data in layer.rank_suggestions.items():
                        if policy == "error":
                            ...
                            if policy == "oht" and "tau" in data:
                                print(
                                    f"        tau={data['tau']:.4f}, omega={data['omega']:.2f}, beta={data['beta']:.3f}"
                                )
                    ...
                    print("   ❌ No rank suggestions found")
                print()
    ...
    result = audit_lora_state_dict(
        state_dict, rank_policies=["optimal_hard_threshold", "entropy_effective", "knee_elbow"]
    )
