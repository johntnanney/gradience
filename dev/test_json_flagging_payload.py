...
def test_json_flagging_rationale():
    ...
    for layer_data in analysis["all_layers_with_disagreement"]:
        ...
        threshold_str = (
            f"{spread_threshold:.1f}" if isinstance(spread_threshold, (int, float)) else str(spread_threshold)
        )
        ...
        threshold_str = (
            f"{uniform_mult_threshold:.1f}×"
            if isinstance(uniform_mult_threshold, (int, float))
            else str(uniform_mult_threshold)
        )
        ...
        if meets_quantile is not None:
            ...
            print("  Quantile check: N/A (flat distribution or condensed format)")
