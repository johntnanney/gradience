...
def run_minimal_training(out_dir: Path) -> bool:
    ...
    try:
        ...
        cmd = [
            "python3",
            "examples/vnext/toy_lora_run.py",
            "--out",
            str(out_dir),
            "--device",
            "cpu",
...
def test_udr_in_bench_context():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            base_norms = {
                "distilbert.transformer.layer.0.attention.q_lin": {"sigma_max": 12.5, "fro_norm": 85.3},
                "distilbert.transformer.layer.0.attention.v_lin": {"sigma_max": 11.8, "fro_norm": 79.2},
                "distilbert.transformer.layer.1.attention.q_lin": {"sigma_max": 10.9, "fro_norm": 76.8},
                "distilbert.transformer.layer.1.attention.v_lin": {"sigma_max": 11.3, "fro_norm": 81.1},
            }
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, env={**os.environ, "PYTHONPATH": str(repo_root)}
            )
        ...
        try:
            ...
            print("   ✅ Audit metrics appended to telemetry")
            ...
            if not audit_events:
                print("   ⚠️  No lora_audit events found in telemetry")
            else:
                ...
                for event in audit_events:
                    ...
                    print("   ⚠️  No UDR metrics found in telemetry events")
        ...
        try:
            ...
            if udr_paths:
                ...
                print("   ⚠️  No UDR metrics found in monitor summary")
