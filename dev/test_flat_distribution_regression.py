...
def test_flat_distribution_regression():
    ...
    print(
        f"Relative spread: {((max(l.frob_sq**0.5 for l in layers) - min(l.frob_sq**0.5 for l in layers)) / base_energy * 100):.1f}%"
    )
    ...
    assert flagged_count == 0, (
        f"REGRESSION FAILURE: Expected 0 high-impact layers in flat distribution, got {flagged_count}"
    )
    ...
    print("   • Flat distribution correctly detected")
    print("   • No layers incorrectly flagged as high-impact")
...
def test_broken_logic_simulation():
    ...
    print("Broken logic (gate disabled):")
    ...
    print("  Correctly detects flat distribution")
    ...
    print("  • Gate prevents meaningless percentile rankings in flat distributions")
...
def test_edge_case_variations():
    ...
    test_cases = [
        {"name": "Very tight clustering (±0.5%)", "base_energy": 10.0, "variance": 0.005, "n_layers": 6},
        {"name": "Large N with tight clustering (±1%)", "base_energy": 3.0, "variance": 0.01, "n_layers": 12},
        {"name": "High energy, tight clustering (±1.5%)", "base_energy": 20.0, "variance": 0.015, "n_layers": 8},
    ]
