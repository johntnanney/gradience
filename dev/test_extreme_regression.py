...
def test_extreme_flat_case():
    ...
    print("Created extreme test case:")
    for layer in layers:
    ...
    for layer in layers:
        ...
        print(
            f"  {layer.name}: energy_share={energy_share:.6f}, uniform_mult={uniform_mult:.6f} {'← highest' if is_highest else ''}"
        )
    ...
    assert is_flat, "REGRESSION FAILURE: Should detect flat distribution"
    assert max_uniform_mult < 1.5, f"REGRESSION FAILURE: Max uniform mult should be < 1.5, got {max_uniform_mult}"

    print("  ✅ Correctly flagged 0 layers (no false positives)")
    ...
    for i, layer in enumerate(layers):
        ...
        spread = max([s["k"] for s in layer.rank_suggestions.values()]) - min(
            [s["k"] for s in layer.rank_suggestions.values()]
        )
...
def test_with_different_distributions():
    ...
    test_cases = [
        {"name": "All identical energies", "energies": [1.0] * 8, "expected_max_mult": 1.0},
        {
            "name": "Tiny variation (±0.001)",
            "energies": [1.0 + i * 0.0001 for i in range(-4, 4)],
            "expected_max_mult": 1.001,
        },
        {"name": "Single outlier at +0.1%", "energies": [1.0] * 9 + [1.001], "expected_max_mult": 1.001},
    ]
...
if __name__ == "__main__":
    ...
    print("  • Current gate logic correctly prevents false positives: YES")
    print("  • Various flat distribution patterns handled correctly: YES")
    print("\n🛡️ This test provides ironclad protection against flat distribution bugs!")
    print("   If this test ever fails, someone has broken the uniform multiplier gate.")
