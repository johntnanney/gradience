...
def test_focus_set_structure():
    ...
    required_fields = ["high_impact_layers", "recommended_focus_n", "focus_strategy", "message", "distribution_type"]
    ...
    assert len(high_impact_layers) == recommended_focus_n, (
        f"Inconsistent counts: {len(high_impact_layers)} != {recommended_focus_n}"
    )
    ...
    print("  ✓ All layer names are valid strings")
    ...
    assert high_impact_layers == expected_order, (
        f"Focus set order doesn't match flagged layer order: {high_impact_layers} vs {expected_order}"
    )
    print("  ✓ High-impact layers correctly ordered by priority_score")
...
def test_flat_distribution_focus():
    ...
    assert focus_set["focus_strategy"] == "top_disagreement_priority", "Expected top_disagreement_priority strategy"
    assert focus_set["recommended_focus_n"] <= 3, (
        f"Flat distribution should recommend ≤3 layers, got {focus_set['recommended_focus_n']}"
    )

    print("  ✓ Flat distribution correctly identified")
    ...
    print("  ✓ Message correctly explains flat distribution")
...
def test_no_disagreement_focus():
    ...
    layers = [
        MockLayer(
            "unimportant_layer_1",
            {"energy_threshold": {"k": 6, "confidence": 0.90}, "knee_elbow": {"k": 3, "confidence": 0.85}},
            frob_sq=0.1,
        ),
        MockLayer(
            "unimportant_layer_2",
            {"energy_threshold": {"k": 5, "confidence": 0.90}, "knee_elbow": {"k": 2, "confidence": 0.85}},
            frob_sq=0.05,
        ),
    ]
    ...
    if focus_set["distribution_type"] == "flat":
        ...
        print("  ✓ No high-impact layers correctly identified")
...
def test_bench_integration_scenarios():
    ...
    single_layer = [
        MockLayer(
            "critical_layer",
            {"energy_threshold": {"k": 8, "confidence": 0.90}, "knee_elbow": {"k": 2, "confidence": 0.85}},
            frob_sq=100.0,
        ),
        MockLayer(
            "normal_layer",
            {"energy_threshold": {"k": 4, "confidence": 0.90}, "knee_elbow": {"k": 3, "confidence": 0.85}},
            frob_sq=5.0,
        ),
    ]
    ...
    multiple_layers = [
        ...
        MockLayer(
            "critical_layer_2",
            {"energy_threshold": {"k": 7, "confidence": 0.90}, "knee_elbow": {"k": 3, "confidence": 0.85}},
            frob_sq=45.0,
        ),
        MockLayer(
            "critical_layer_3",
            {"energy_threshold": {"k": 6, "confidence": 0.90}, "knee_elbow": {"k": 2, "confidence": 0.85}},
            frob_sq=40.0,
        ),
        MockLayer(
            "normal_layer",
    ...
    print("```python")
    print("# In Bench variant configuration:")
    print("if variant.type == 'per_layer' or variant.type == 'rank_pattern':")
    print("    focus_layers = audit_result['disagreement_focus_set']['high_impact_layers']")
    print("    if focus_layers:")
    print("        # Restrict to high-impact layers only")
    print("        variant.target_layers = focus_layers")
    print("    else:")
    print("        # Fall back to uniform suggestions")
    print("        variant.use_uniform_suggestions()")
    print("```")
