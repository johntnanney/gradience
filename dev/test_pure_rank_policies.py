...
class TestKneePolicy:
    ...
    def test_knee_clear_elbow(self):
        ...
        assert any(key in result.details for key in ["elbow_index", "knee_index"]), (
            f"Missing elbow metadata in {result.details.keys()}"
        )
    ...
    def test_knee_insufficient_data(self):
        ...
        assert (
            "reason" in result.details and "insufficient" in str(result.details.get("reason", "")).lower()
        ) or "flat_spectrum" in result.details, f"Expected insufficient data indication in {result.details}"
