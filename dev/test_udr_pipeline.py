...
def create_test_base_norms() -> dict[str, dict[str, float]]:
    ...
    for layer_name in ["q_proj", "k_proj", "v_proj", "gate_proj"]:
        prefix = (
            f"base_model.layers.0.self_attn.{layer_name}"
            if "proj" in layer_name and layer_name != "gate_proj"
            else f"base_model.layers.0.mlp.{layer_name}"
        )
        ...
        norms[prefix] = {"sigma_max": float(base_sigma), "fro_norm": float(base_fro)}
...
def run_udr_test():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            from gradience.vnext.audit.lora_audit import compute_udr_metrics, compute_update_norms
