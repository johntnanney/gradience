...
def test_json_size_reduction():
    ...
    print("  • 1 high-impact layer (will be flagged)")
    ...
    assert flagged_count_full == flagged_count_flagged_only, (
        f"Flagged count mismatch: {flagged_count_full} vs {flagged_count_flagged_only}"
    )
    print(f"  ✓ Same flagging results: {flagged_count_full} layers flagged in both modes")

    # Both should process same total layers
    assert total_layers_full == total_layers_flagged_only, (
        f"Total layer count mismatch: {total_layers_full} vs {total_layers_flagged_only}"
    )
    ...
    if non_flagged_full and non_flagged_flagged_only:
        ...
        assert required_condensed_fields.issubset(condensed_keys), (
            f"Missing required condensed fields: {required_condensed_fields - condensed_keys}"
        )
...
def test_edge_cases():
    ...
    high_importance_layers = [
        ...
        MockLayer(
            "layer2",
            {"energy_threshold": {"k": 6, "confidence": 0.90}, "knee_elbow": {"k": 3, "confidence": 0.80}},
            frob_sq=80.0,
        ),
    ]
    ...
    if flagged_count == total_count:
        print("  ✓ All layers flagged - no condensing needed")
    ...
    low_importance_layers = [
        ...
        MockLayer(
            "layer2",
            {"energy_threshold": {"k": 4, "confidence": 0.90}, "knee_elbow": {"k": 2, "confidence": 0.80}},
            frob_sq=0.1,
        ),
    ]
    ...
    for layer_data in analysis["all_layers_with_disagreement"]:
        ...
        if "failed_reasons" in rationale:
            print(
                f"  ✓ Layer {layer_data['layer_name']}: condensed rationale with {len(rationale['failed_reasons'])} failure reasons"
            )
