...
def create_fallback_scenario():
    ...
    audit_data = {
        ...
        "computed_with": {"gradience_version": "0.5.0", "git_sha": "2277c7b7fe3f"},
        "policy_disagreement_analysis": analysis,
        "adapter_info": {"total_layers": len(layers), "model_name": "fallback_test_model"},
    }
...
if __name__ == "__main__":
    ...
    print("✅ Created fallback_scenario_audit.json")
    print(f"   Flagged layers: {flagged}")
    print(f"   Total disagreement layers: {total_disagreement}")

    if flagged == 0 and total_disagreement > 0:
        print("   Perfect for testing fallback → top-N by priority_score")

    print("\n🧪 Test the golden pattern fallback:")
    print("   python3 golden_downstream_pattern.py fallback_scenario_audit.json")
    print("   python3 golden_downstream_pattern.py fallback_scenario_audit.json --top-n 2")
