...
**Date:** 2026-04-25 (lock date)

**Status:** **LOCKED at 2026-04-25.** No data has been collected under this protocol. All research decisions resolved; all software pins, dataset hashes, prompt content hashes, and admissibility statuses fixed. Config hash `65fdd1c2` (full SHA-256: `65fdd1c29d63a78b85c128d56888240967c1b7cf5274cedf8bfc01cc7a0242d9`). Subsequent changes to spec-committed parameters require a `deviations.md` entry per §11. See `LOCK_NOTES.md` for the lock-audit record and the full provenance manifest.
...
- **v1.1-LOCKED (2026-04-25):** all operational items resolved. (5) 24 prompt files sourced from canonical community repositories: `EleutherAI/lm-evaluation-harness` at commit `c1c4bea3`, `stanford-crfm/helm` at commit `11937097`, plus benchmark-author repos and one published variant (Brown et al. 2020 GPT-3 paper for Winogrande P3). Six byte-identical-template identity classes documented in `configs/prompts.yaml`; the canonical-source ecology produces nontrivial template convergence (notably P1↔P2 in 4 of 6 benchmarks where lm-eval-harness inherits the authors' template directly). (6) Model HF revisions pinned: pythia-410m@9879c9b5, pythia-1.4b@fedc38a1, Qwen2.5-1.5B-Instruct@989aa798. (7) Dataset version hashes pinned: ai2_arc@210d026f, hellaswag@218ec52e, truthful_qa@741b8276, mmlu@c30699e8, winogrande@01e74176, gsm8k@740312ad. (8) Prompt content hashes computed via SHA-256 on each prompt file; all 24 entries promoted from `admissibility_status: draft` to `admissibility_status: locked`. (9) Config-validation pass clean: 0 errors, 0 warnings, config_hash `65fdd1c2`. The protocol is now lock-eligible; data collection may begin.

- **v1.1.1-LOCKED (2026-04-25):** Phase 3 dry-run caught two issues before any data was collected: (a) `benchmarks.yaml` had `item_id_field: qID` for Winogrande, but the HF dataset uses `sentence` as the natural item identifier (Winogrande has no numeric ID column); (b) `02_draw_fewshot_examples.py` had a tier filter that excluded GSM8K (secondary tier) from few-shot draws, which would have failed at Phase 4 inference time. Both fixes applied; locked configs re-validated. New config hash `89ce3f1f`. 136 fewshot draws produced and locked at `preregistration/appendices/fewshot_draws_LOCKED.json`. Manifests, prompt manifest, and validation report all clean. Test suite: 133/133 passing. See `LOCK_NOTES.md` "Amendment: v1.1.1-LOCKED" section for full audit. The v1_1_LOCKED tag remains in the repository as the historical record of the catch; v1_1_1_LOCKED is the executable lock.

- **v1.1.2-LOCKED (2026-04-26):** Two daily research reviews + early Phase 4 GPU-run data motivated a regime-split resolution of D-09 (LPM-not-logistic deviation). The 2026-04-26 second-pass review surfaced NIST AI 800-3 (Feb 2026), which formally endorses GLMM for AI variance decomposition; preliminary GPU output showed ~half the cells (parse-failure-dominated G&P cells on base models) sit outside the 0.15–0.85 accuracy range where LPM and GLMM agree. Resolution: added `tolerance.parse_failure_threshold: 0.30` to `analysis_config.yaml`; cells with median parseability below threshold route to sample-SD-based tolerance (D-07 pattern) instead of variance-components SEM, marked with `regime: parse_failure_dominated` in `tolerance_by_cell.csv`. The LPM-vs-GLMM regime is now explicitly limited to cells where the decomposition is methodologically meaningful. New config hash `fbc4a5dd`. Pre-registered hypothesis tests unchanged; the seven load-bearing commitments (benchmarks, MMLU panel, prompts, HF revisions, decision rules, bootstrap, data hierarchy) are unchanged. Test suite: 181 passed, 2 skipped. See `LOCK_NOTES.md` "Amendment: v1.1.2-LOCKED" section for full audit. v1_1_2_LOCKED supersedes v1_1_1_LOCKED as the executable lock.
