...
\textbf{Across-rule tolerance schedule.} The §\ref{sec:tolerance-schedule-results} schedule is the rule-conditioned schedule: per (benchmark, model, scoring rule), it licenses precision conditional on a declared scoring rule. A complementary across-rule schedule, treating scoring rule as part of the admissible measurement universe rather than as a fixed cell-defining dimension, would license precision for the field's common reporting practice of citing benchmark accuracy without naming the scoring rule. The two schedules answer different questions: the rule-conditioned schedule is what the present paper develops and reports; the across-rule schedule would express the additional uncertainty introduced when a reported number does not commit to a scoring-rule choice. A larger follow-up could report both schedules side by side across a broader model and benchmark panel, making the prescriptive output more directly useful to the field's reporting practice when scoring rule is left unspecified.
