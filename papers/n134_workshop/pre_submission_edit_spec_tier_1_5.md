...
> **Post-execution annotation (2026-04-26 cite-verification pass).** The "NeurIPS 2025 construct-validity systematic review" cited below as `\citet{reuel2025measuring}` was a misattribution. The paper at arXiv:2511.04703 is by Andrew M. Bean (Oxford) and 41 co-authors; Anka Reuel is not in the author list. The misattribution likely arose by conflation with Reuel et al. NeurIPS 2024 *BetterBench*, a different paper. The bib key was corrected to `bean2025measuring` and the manuscript's `\citet` updated on 2026-04-26 (see `references.bib` and `draft_v2_thesis_b.tex` markers of the same date). The body of this spec is preserved as the historical record of what was specified at the Tier 1.5 pass; it is not rewritten. Anyone reading this spec for editorial-precedent purposes should treat the `reuel2025measuring` references below as referring to the corrected `bean2025measuring` entry.
