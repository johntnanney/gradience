...
% EDIT: 2026-04-26 — pre-submission cite verification: \citet key updated
% EDIT: from `reuel2025measuring` (misattribution caught via arXiv API) to
% EDIT: `bean2025measuring` reflecting the actual first author. arXiv:2511.04703
% EDIT: has 42 authors; first author is Andrew M. Bean (Oxford). Anka Reuel
% EDIT: is not in the author list. Likely conflation with Reuel et al.
% EDIT: NeurIPS 2024 BetterBench, a different paper. Bib entry renamed
% EDIT: in references.bib with the same date marker.
...
environment-sensitive numerical properties of the statistic at the operative
...
statistic-level reproducibility each contribute, and which one
...
environment-sensitive floating-point reproducibility of approximately $\pm 0.01$ at
...
it is about the statistic's environment-sensitive reproducibility on residuals at this
...
% EDIT: per-flip algebra. Original text claimed Δρ per rank-pair flip
% EDIT: ≈ 2/(n(n²-1)); empirical verification at n=45 (adjacent swap in
% EDIT: one variable, scipy.stats.spearmanr) gives Δρ ≈ 1.32e-4, which
% EDIT: matches the 12/(n(n²-1)) coefficient from the standard no-ties
% EDIT: Spearman formula, not 2/(n(n²-1)). Rather than substitute a
% EDIT: new precise coefficient (which would require a more careful
% EDIT: near-tie-propagation analysis than the paper does), retreat to
% EDIT: qualitative wording. The paper's argument — that many small
% EDIT: rank perturbations accumulate to order-10⁻² drift at this n —
% EDIT: is preserved; fake precision is dropped.
...
The rank-on-residuals property is
...
framework's reproducibility discipline. The reliability estimate and the
...
% EDIT: 2026-04-26 — pre-tarball pass: the "Full trace in the
% EDIT: supplementary materials" pointer was inaccurate; the trace document
% EDIT: was deliberately excluded from the supplementary bundle (see
% EDIT: supplementary/README.md "Not attached" list). The four-tier summary
% EDIT: below is itself the load-bearing trace for review purposes.
Summary of the reproducibility check performed on the committed
state of the worked example, organized by the four-tier verification
protocol specified in Section~\ref{sec:rank-observation} and in the
study's methodology convention.
...
% EDIT: 2026-04-26 — pre-tarball pass: dropped the
% EDIT: "documented in the companion reproducibility-check trace" clause;
% EDIT: that trace document is not in the supplementary bundle. The two
% EDIT: gaps are documented inline in this paragraph.
...
Phase 5 per-adapter factor files are pod-only. Neither gap affects
qualitative claims.
...
% EDIT: 2026-04-26 — pre-tarball pass: dropped the "supplementary incident
% EDIT: log" / "supplementary technical report (§8 therein)" pointer; both
% EDIT: documents were deliberately excluded from the supplementary bundle
% EDIT: (see supplementary/README.md "Not attached" list). The D1–D6
% EDIT: summary above is the load-bearing deviation record for review
% EDIT: purposes; further detail is available via OpenReview correspondence
% EDIT: should reviewers request it.
method-comparison numbers were committed. The above D1--D6 summaries
are the load-bearing deviation record for review purposes.
...
% EDIT: 2026-04-26 — pre-tarball pass: dropped the post-hoc-pip-freeze
% EDIT: claim; that file is not in the supplementary bundle (the bundle
% EDIT: was trimmed to load-bearing artifacts only).
captured).
...
% EDIT: 2026-04-26 — pre-tarball pass: artifact list rewritten to match
% EDIT: what the supplementary bundle actually ships per
% EDIT: supplementary/README.md. Dropped figure-generation scripts and
% EDIT: reproducibility-check trace (both deliberately excluded from the
% EDIT: bundle). Specified the four top-level analytical JSONs and the
% EDIT: 24-adapter raw-audit count for clarity.
