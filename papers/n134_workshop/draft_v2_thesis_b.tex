...
diagnostic metric as a point estimate to three or four decimal places
without any accompanying description of its measurement properties.
The LoRA-merging literature offers one recent example: predictive
correlations between spectral diagnostics and merge outcomes are
reported with two or three significant figures
\citep{zhou2026demystifying,rahamim2026mergeability} without
cross-seed reliability coefficients, without explicit tolerance
schedules for the reported decimals, and without pre-registered
confound decomposition that would distinguish the diagnostic's
predictive power from that of simpler alternatives such as
task-family identity. The pattern is not confined to the merging
subfield; it recurs across interpretability score reports, training-
dynamics diagnostics, and capability evaluation scores.

Three specific gaps follow from this pattern. Reliability
coefficients in the classical-test-theory sense --- ICC, SEM,
coefficient alpha or its distribution-based analogues --- are nearly
absent from ML diagnostic reports. Point estimates are quoted to
three or four decimals without uncertainty bounds calibrated to the
actual dependence structure of the data. Confound decomposition is
rare, and when present is usually conducted post-hoc rather than
pre-registered. Each of these gaps on its own is repairable by a
careful reviewer; collectively, they produce a literature in which
a reader who wants to know how much credence to place in a reported
value has no reliable way to calibrate.
...
An ML diagnostic claims to measure something. Our framework requires
the paper to state that something explicitly --- call it the
\emph{theoretical object} of the measurement --- and to defend, as
a separate argumentative step, the \emph{operationalization} that
connects the reported computation to the theoretical object. The ML
literature's typical failure mode is conflating these two: a paper
that says ``our spectral alignment score predicts merge outcomes'' is
usually reporting results about the operationalization (the specific
computation from weights) as if they were results about the
theoretical object (the geometric relationship between two adapters'
learned updates).

The distinction matters because different operationalizations of the
same theoretical object can produce different predictive patterns on
the same data. N07/N132's subspace-principal-angle-based overlap
metric and N133/N134's singular-value-weighted cosine alignment both
claim to measure the same theoretical object (spectral similarity
between two adapters' weight-space updates), but their raw values are
not on comparable scales, and cross-study comparisons of the raw
values are unsound without metric-family harmonization. Pre-registered
articulation of the theoretical object and its operationalization is
the discipline that forces such harmonization to be explicit.

\subsection{Reliability}
\label{sec:framework-reliability}

For a diagnostic intended to support predictive claims, the
test-retest behavior of the score across seeds, calibration data
samples, or implementation details is not incidental: it bounds the
precision of any predictive claim the score can support. An ML
diagnostic reported as ``$\rho = 0.573$'' without an accompanying
cross-seed ICC or equivalent reliability coefficient is implicitly
claiming that the score is perfectly reliable, which is almost never
true. The framework's recommendation is that ICC (or a distribution-
based equivalent for score types where ICC is not well-defined) be
reported for any diagnostic used to support quantitative predictive
claims.

We make the recommendation specific rather than hortatory. For a
score computed per-adapter, cross-seed ICC is computed from three or
more seeds per task; at two seeds per task (a common convention) the
ICC estimate has wide confidence intervals that are themselves
informative. For a score computed per-pair of models, the analog is
cross-seed-pair ICC across pairs that should be semantically
equivalent (same task-pair, different random seeds); the N134 design
uses three seeds per task specifically to enable this estimate.

\subsection{Bounded precision}
\label{sec:framework-precision}

A diagnostic value reported as ``$-0.533$'' to three decimals is
making an implicit claim that the third decimal is meaningful. This
claim is often defensible from sampling variability alone but is
rarely defensible when implementation sensitivity, cross-environment
reproduction, and intrinsic properties of the statistic at the
sample size are considered. Bounded precision is the discipline of
stating explicitly what the uncertainty is on the reported decimal
places, broken out by source.

Section~\ref{sec:rank-observation} develops a concrete instance: the
worked example's headline statistic has intrinsic floating-point
precision of approximately $\pm 0.01$ at $n = 45$, meaning its third
decimal is not a real claim regardless of what sampling variability
the bootstrap confidence interval reports.

\subsection{Confound decomposition with pre-registered decision rules}
\label{sec:framework-confound}

The final component is the discipline of distinguishing what a
...
structure of the data. The worked example in
Section~\ref{sec:empirical} makes the distinction concrete: 88\% of
merge-outcome variance is captured by task-family-pair identity
alone, leaving 12\% within which a geometric diagnostic must operate
to clear a pre-registered $\Delta R^2 \geq 0.10$ threshold. Without
the family-decomposition step, a raw correlation would conflate
family identity with geometric prediction; with it, the residual
predictive power is legible directly.

Pre-registration of the decomposition is what distinguishes the
framework from post-hoc confound analysis. Post-hoc decomposition
following a failed main analysis reads as an excuse; pre-registered
decomposition is a commitment that the diagnostic must clear
\emph{before} seeing the data. The worked example commits to its
family-residualization, its $\Delta R^2$ threshold, and its sign
constraint on partial Spearman $\rho$ before data collection; we
return to this in Section~\ref{sec:applying}.
...
estimate requires. We report the instrument validation in
Appendix~\ref{app:reliability}; for the purpose of the pre-registered
H1 test, the relevant observation is that the design was calibrated
to support this estimate before data collection began, not that the
estimate was examined post-hoc to reassure reviewers.
...
\citet{messick1995validity} on construct validity as unified-theory
framework, and \citet{cronbachmeehl1955} on construct validity and
...
\emph{[TODO: insert cross-seed ICC computation from the 24 adapters
(3 seeds $\times$ 8 tasks) as the instrument-validation step this
paper's framework requires; result populates this appendix and
grounds Section~\ref{sec:framework-reliability}. One-day analysis on
committed data. Scheduled as an early-revision action per
\texttt{revision\_notes.md} RN-012 immediate next actions.]}
