...
5. **Confirmatory replications** — B-P1/B-P2/B-P4 from report §4; three-architecture table with DistilBERT + DeBERTa + Mistral-7B; emphasis on task-boundary detection as the Gradience program's most robust architecture-general finding. Target 0.5 page + Figure 2.

6. **Four-method comparison** — drawn from report §6. Table of four methods with Spearman ρ, p, retained-set mean degradation, bootstrap CI. Prose emphasizing the regime null (no method is significant; 88% of variance is family-identity). Target 0.75 page + Figure 3.

7. **Discussion** — condensed from report §7. Three-architecture replication as the consolidation; per-pair risk regression outside current measurement envelope; four candidate follow-up programs (larger N, activation-informed, intrinsic-mergeability, training-time intervention); brief note on the wrong-signed ρ as hypothesis-generating residual. Target 1 page + Figure 4.
...
Figure plan (generated by scripts in `scripts/n134/figures/`, output in `papers/n134_workshop/figures/`):

- **F1.** H1 decision plot: S_H1 vs max_degradation with family coloring, raw + partial regression lines.
- **F2.** Three-architecture same/cross alignment comparison: DistilBERT, DeBERTa, Mistral-7B (N133), Mistral-7B (N134).
- **F3.** Four-method forest plot: retained-set mean degradation with bootstrap CIs; random-baseline reference line.
- **F4.** Layer-depth trend in same/cross alignment ratio on Mistral-7B N134.
