...
def main():
    ...
    print(
        f"   • Safe uniform baseline = ≥ {policy.get('min_pass_rate', 2 / 3):.0%} seeds PASS AND worst seed Δ ≥ {policy.get('min_worst_delta', -0.025):+.1%}"
    )
    print("   • Preferred baseline = highest compression among safe candidates")
    print("   • If no candidate is safe → recommend per-layer adaptive as default")
    print()
    ...
    safe_baselines = {name: safety for name, safety in safety_results.items() if safety["verdict"] == "SAFE"}
    ...
    if not safe_baselines:
        ...
        safe_ranked = sorted(safe_baselines.items(), key=lambda x: x[1]["mean_reduction"], reverse=True)
        ...
        print()
        print(
            f"**Rationale:** Highest compression ({preferred_safety['mean_reduction']:.1f}% reduction) among policy-compliant candidates"
        )
        print()
        ...
        for baseline, safety in safe_ranked:
            ...
            print(
                f"   {baseline}: {pass_icon} {safety['pass_fraction']} pass rate | {delta_icon} {safety['worst_case_delta']:+.3f} worst delta"
            )
        ...
        print("→ **Avoid:** All non-compliant uniform approaches")
        print("→ **Monitor:** Validate safety policy holds on production workloads")
