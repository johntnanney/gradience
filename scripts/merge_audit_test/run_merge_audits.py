...
def run_all_pairs():
    ...
    for label, name_a, name_b, hypothesis in PAIRS:
        ...
        report, elapsed, error = run_single_audit(
            label, path_a, path_b, out_dir, thresholds_name="default"
        )
        ...
        print(f"  Verdicts:  {agg['n_safe']} safe, {agg['n_redundant']} redundant, "
        ...
        report_c, elapsed_c, error_c = run_single_audit(
            label, path_a, path_b, None, thresholds_name="conservative"
        )
        if report_c:
            agg_c = report_c.aggregate
            print(f"  Conservative: {agg_c['overall_verdict'].upper()} "
...
if __name__ == "__main__":
    ...
    for r in results:
        if r["status"] == "ok":
            print(f"  {r['label']:<35s} {r['elapsed_s']:>5.1f}s {r['n_layers']:>6d} "
