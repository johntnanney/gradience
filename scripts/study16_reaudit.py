...
class PairAuditResult:
    ...
    old_layer_verdict_counts: Optional[Dict[str, int]] = None
    ...
    if old_audit_json.exists():
        try:
            ...
            if old_verdict and old_verdict != verdict:
                ...
                print(f"  [CHANGED: {old_verdict}→{verdict}, "
                      f"imbalanced layers: {old_imb}→{new_imb}]", end="")
            elif old_verdict:
                old_imb = old_layer_counts.get("imbalanced", 0)
                new_imb = verdict_counts.get("imbalanced", 0)
                if old_imb != new_imb:
...
def write_summary(
    ...
    if has_diff:
        ...
        layer_changed = [r for r in results if r.old_layer_verdict_counts is not None
        ...
        if changed or layer_changed:
            ...
            for r in results:
                ...
                layers_changed = (r.old_layer_verdict_counts is not None
                                  and r.old_layer_verdict_counts != r.verdict_counts)
                ...
                lines.append(
                    f"| {r.label_a} x {r.label_b} | {old_v} | {new_v} | {old_vc} | {new_vc} |"
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Study 16 Re-Audit: Re-run merge audits with updated verdict engine"
    )
    parser.add_argument(
    ...
    if args.pairs:
        pairs_to_run = [
            p for p in PAIRS
            if any(filt in p.pair_id for filt in args.pairs)
        ]
        if not pairs_to_run:
    ...
    print(f"  Verdict engine: 6-branch (Frobenius imbalance enabled)")
    print(f"  Thresholds: imbalanced_frob={thresholds.imbalanced_frob}, "
