...
def print_status_report(results: Dict[str, any]) -> None:
    ...
    print(f"🔍 Bench Run Status Report")
    print(f"{'='*50}")
    ...
    if results["run_type"] == "multiseed":
        ...
        print(f"📋 Individual Seed Status:")
        for seed in results["seeds"]:
            status_icon = {
                "completed": "✅",
                "stuck_or_running": "⏳", 
...
def main():
    parser = argparse.ArgumentParser(
        description="Check bench run status and detect stuck runs"
    )
    parser.add_argument(
        "output_dir",
        help="Path to bench output directory to check"
    )
    parser.add_argument(
        "--auto-resume",
        action="store_true",
        help="Automatically attempt to resume stuck runs"
    )
    parser.add_argument(
        "--json",
        action="store_true", 
