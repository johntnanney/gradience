...
def pick_representative_seeds(seed_results, n=3):
    ...
    grokked = [(sid, sr) for sid, sr in seed_results.items() if sr["grok_step"] is not None]
...
def fig1_windowed_timeline(data, out_dir):
    ...
    for ax, seed_id in zip(axes, seeds):
        ...
        for metric in CURVATURE + CONTROL:
            ...
            all_windows = segments.get("pre", []) + segments.get("transition", []) + segments.get("post", [])
        ...
        # Grokking line
        if grok_step:
            ax.axvline(x=grok_step, color="red", linestyle="--", linewidth=2, alpha=0.7, label=f"T_grok={grok_step}")

        # White noise reference
        ax.axhline(y=0.5, color="gray", linestyle=":", alpha=0.4)
        ax.text(ax.get_xlim()[0] + 500, 0.52, "α=0.5 (white noise)", fontsize=7, color="gray")

        ax.set_ylabel("DFA α", fontsize=11)
        ax.set_title(f"{seed_id} (grok @ step {grok_step})", fontsize=11, fontweight="bold")
    ...
    fig.suptitle("Windowed DFA Exponents Across Grokking Transition", fontsize=14, fontweight="bold", y=1.01)
    fig.tight_layout()
...
def fig2_pre_post_comparison(data, out_dir):
    ...
    for ax, metric in zip(axes, CURVATURE):
        ...
        # Box + strip plot
        positions = [0, 1]
        bp = ax.boxplot([pre, post], positions=positions, widths=0.5, patch_artist=True, showfliers=False)
        ...
        for p_val, post_val in zip(pre, post):
            ax.plot([0, 1], [p_val, post_val], "-", color="gray", alpha=0.3, linewidth=0.8)

        jitter_pre = np.random.normal(0, 0.05, len(pre))
        jitter_post = np.random.normal(1, 0.05, len(post))
        ax.scatter(jitter_pre, pre, color=COLORS[metric], s=25, zorder=3, alpha=0.7, edgecolor="white", linewidth=0.5)
        ax.scatter(jitter_post, post, color=COLORS[metric], s=25, zorder=3, alpha=0.7, edgecolor="white", linewidth=0.5)
        ...
        ax.set_title(
            f"{NICE[metric]}\nΔα={tr['mean_delta']:+.3f}, d={d:.2f}, p={p:.3g} {sig}", fontsize=10, fontweight="bold"
        )
    ...
    fig.suptitle("DFA Exponent Shift: Pre-Grok vs Post-Grok", fontsize=13, fontweight="bold")
    fig.tight_layout()
...
def fig3_early_warning(data, out_dir):
    ...
    for metric in CURVATURE:
        leads = [lt["lead_steps"] for lt in lead_times if lt["metric"] == metric and lt["detect_step"] is not None]
        if leads:
            ax.hist(
                leads,
                bins=15,
                alpha=0.5,
                color=COLORS[metric],
                label=f"{NICE[metric]} (n={len(leads)})",
                edgecolor="white",
            )

    ax.axvline(x=500, color="red", linestyle="--", linewidth=1.5, label="500-step threshold")
    ax.set_xlabel("Lead Time (steps before grokking)", fontsize=11)
    ax.set_ylabel("Count", fontsize=11)
    ax.set_title(
        f"Early-Warning Detection Lead Times\n"
...
def fig4_raw_signals(data, out_dir):
    ...
    for metric in CURVATURE:
        ...
        if vals:
            ...
            axes[0].semilogy(
                steps, np.abs(vals) + 1e-15, color=COLORS[metric], label=NICE[metric], linewidth=0.7, alpha=0.8
            )
    axes[0].set_ylabel("Curvature (|value|, log)", fontsize=10)
    axes[0].legend(fontsize=8, loc="upper right")
    axes[0].set_title(f"Raw Telemetry: {seed_id} (grok @ step {grok_step})", fontsize=12, fontweight="bold")
    ...
    for metric in CONTROL:
        ...
        if vals:
            vals = np.array([v if v is not None else np.nan for v in vals])
            axes[1].plot(steps, vals, color=COLORS[metric], label=NICE[metric], linewidth=0.7, alpha=0.8)
    ...
    axes[2].text(
        0.5,
        0.5,
        "(val_acc requires all_records — see telemetry.jsonl)",
    ...
    for ax in axes:
        if grok_step:
            ax.axvline(x=grok_step, color="red", linestyle="--", linewidth=1.5, alpha=0.5)
        ax.grid(True, alpha=0.2)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="results/study13/study13_windowed_alphas.pkl")
