...
def generate_public_summary(output_dir: Path) -> str:
    ...
    for seed_dir in sorted(seed_dirs):
        ...
        seed_analyses.append({
            ...
            "decision_trace": decision_analysis,
    ...
    summary.append("")
    summary.append(f"**Model**: Mistral-7B-v0.1 fine-tuned on GSM8K mathematical reasoning")
    ...
    if best_compressions:
        ...
        for i, comp in enumerate(best_compressions):
            ...
            if best_name in comp["compression_candidates"]:
                ...
                summary.append(f"  - Seed {i+1}: {best_name} ({param_red:.1f}% size reduction, {acc_ret:.1f}% accuracy retention)")
