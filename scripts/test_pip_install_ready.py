...
def test_base_install():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Test import
        if not run_cmd(
            f"{activate_cmd} && python -c \"import gradience; print(f'Version: {{gradience.__version__}}')\"",
            "Testing import",
        ):
            return False
...
def test_bench_extras():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        activate_cmd = f"source {venv_path}/bin/activate"
        if not run_cmd(
            f"{activate_cmd} && pip install torch --index-url https://download.pytorch.org/whl/cpu",
...
def test_performance():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not run_cmd(
            f"timeout 15s bash -c '{activate_cmd} && gradience-bench --help'",
            "Testing gradience-bench --help first run",
            timeout=20,
        ):
            return False
        ...
        if not run_cmd(
            f"timeout 5s bash -c '{activate_cmd} && gradience-bench --help'",
            "Testing gradience-bench --help second run",
        ):
            return False
...
def main():
    ...
    parser.add_argument(
        "--test", choices=["base", "bench", "packaging", "performance", "all"], default="all", help="Which test to run"
    )
