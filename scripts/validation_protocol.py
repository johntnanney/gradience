...
class ValidationProtocol:
    ...
    def run_command(
        self, cmd: list[str], cwd: Path | None = None, capture: bool = False
    ) -> subprocess.CompletedProcess:
        """Run command with logging."""
        if self.verbose:
            self.log(f"Running: {' '.join(cmd)}")

        result = subprocess.run(cmd, cwd=cwd or self.protocol_dir, capture_output=capture, text=True, check=False)
    ...
    def create_tiny_training_config(
        self, output_dir: Path, rank: int, rank_pattern: dict[str, int] | None = None
    ) -> Path:
    ...
    def train_probe(self) -> Path:
        ...
        if result.returncode == 0:
            ...
            self.log("✗ Probe training failed")
...
""",
    ...
    def generate_suggestions(self, probe_dir: Path) -> dict[str, Any]:
        ...
        try:
            ...
            self.log("✓ Generated suggestions:")
...
def main():
    ...
    parser.add_argument(
        "--model",
        default="tiny-distilbert",
        choices=["tiny-bert", "tiny-distilbert", "tiny-gpt2"],
        help="Model to use for validation",
    )
    parser.add_argument(
        "--dataset", default="tiny", choices=["tiny", "cola", "sst2"], help="Dataset to use for validation"
    )
    parser.add_argument("--probe-r", type=int, default=16, choices=[8, 16, 32], help="Probe rank for initial training")
    parser.add_argument(
        "--base-dir", type=Path, default="./validation_runs", help="Base directory for validation outputs"
    )
    ...
    protocol = ValidationProtocol(
        base_dir=args.base_dir, model=args.model, dataset=args.dataset, probe_r=args.probe_r, verbose=args.verbose
    )
