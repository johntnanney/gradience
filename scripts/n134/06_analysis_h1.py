"""N134 primary H1 analysis + confirmatory replications.
...
def plot_h1_scatter(
    ...
    ax.set_title(
        f"N134 H1: S_H1 vs merge degradation\n"
...
def plot_bootstrap_distribution(boot_results: dict, out_path: Path) -> None:
    ...
    fig.suptitle(f"N134 H1 block-bootstrap CIs ({boot_results['n_valid_boots']} resamples)", fontsize=13)
    fig.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
...
def plot_replications(bp1: dict, bp2: dict, bp4: dict, out_path: Path) -> None:
    ...
    fig.suptitle("N134 confirmatory replications (non-gating)", fontsize=13)
    fig.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
...
def main() -> None:
    ...
    # ---- Save JSON ----
    result = {
        "experiment": "N134 H1: O-module depth-weighted alignment",
    ...
    print("\n" + "=" * 74)
    print(f"  N134 H1 result: {'CONFIRMED' if h1_confirmed else 'NOT CONFIRMED'}")
    print("=" * 74)
