...
class TinyTransformer(nn.Module):
    def __init__(self, vocab_size, d_model=128, nhead=4, num_layers=2):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model, nhead, d_model * 4, dropout=0.0, batch_first=True)
...
while step < max_steps:
    ...
    if step % 500 == 0:
        ...
        print(
            f"{step:>6} {loss.item():>8.4f} {train_correct * 100:>7.1f}% {test_acc * 100:>7.1f}% {agg.get('mean_kappa', 0):>10.1f} {agg.get('mean_effective_rank', 0):>8.1f}"
        )
