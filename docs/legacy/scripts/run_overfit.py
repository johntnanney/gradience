...
for step in range(2001):
    ...
    if step % 100 == 0:
        ...
        # Spectra
        spectra = [
            compute_full_spectrum(m.weight, f"layer_{i}", step) for i, m in enumerate(model) if isinstance(m, nn.Linear)
        ]
        ...
        print(
            f"{step:>5} {train_acc * 100:>7.1f}% {test_acc * 100:>7.1f}% {loss.item():>8.4f} {agg.get('mean_kappa', 0):>10.1f} {agg.get('mean_effective_rank', 0):>8.1f}"
        )
