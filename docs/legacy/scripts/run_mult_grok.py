...
os.makedirs("./experiments/grokking_mult", exist_ok=True)

train = ModularDataset(p, "mult", "train")
test = ModularDataset(p, "mult", "test")
...
from gradience.research import aggregate_layerwise_spectra, compute_full_spectrum

results = []
step = 0
max_steps = 150000

print("=== MULTIPLICATION MOD 97 (Harder Grokking) ===\n")
...
while step < max_steps:
    ...
    if step % 500 == 0:
        ...
        print(
            f"Step {step:>6} | Loss: {loss.item():.4f} | Test: {test_acc * 100:.1f}% | Rank: {agg.get('mean_effective_rank', 0):.1f}"
        )
...
with open("./experiments/grokking_mult/results.json", "w") as f:
    json.dump(results, f, indent=2)
print("\nSaved to ./experiments/grokking_mult/results.json")
