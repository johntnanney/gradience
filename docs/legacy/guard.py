...
class GuardConfig:
    ...
    # Logging
    log_path: str | None = None  # If set, write events to JSONL
...
class GuardState:
    ...
    last_known_good_loss: float | None = None
...
class Guard:
    ...
    def _check_integrity(self, step: int, loss: float) -> CorruptionType | None:
    ...
    def _handle_corruption(self, step: int, loss: float, corruption: CorruptionType) -> "GuardStatus":
    ...
    def _attempt_recovery(self, step: int, corruption: CorruptionType) -> "GuardStatus":
    ...
    def _prune_old_rollbacks(self, current_step: int):
        ...
        self._state.recent_rollback_steps = [s for s in self._state.recent_rollback_steps if s > cutoff]
...
class GuardStatus:
    ...
    corruption_detected: CorruptionType | None = None
    action_taken: str | None = None
    rolled_back_to_step: int | None = None
    reason: str | None = None
    muon_ratio: float | None = None  # ρ = λ × σ_max
    ...
    for _cand in (
        ...
        class IntegrityTracker:
            ...
            def record(
                ...
                self.events.append(
                    _IntegrityEvent(step=step, code=code, severity=severity, message=message, context=context or {})
                )
            ...
            def update(self, status: Any, step: int | None = None) -> None:
                ...
                if corrupted or should_abort:
                    ...
                    self.record(
                        code=str(code), severity="warning", message=str(reason), step=step, context=details or {}
                    )
