...
4. corpus identity hardening follow-through status recorded
...
- Decision id: `<calibration-202605-XX>`
...
- mixed-evidence inventory count: `<int>`
- block/non-block split in mixed-evidence slice: `<int>/<int>`
...
- non-singleton neighborhoods observed: `<yes/no>`
- if yes, count and run ids: `<line>`
- if no, implication for calibration confidence: `<line>`

### Identity hardening follow-through

- status: `<implemented | in-flight | blocked>`
- effect on adapter-instance counting trust: `<2-3 sentences>`
