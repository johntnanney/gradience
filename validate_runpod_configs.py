...
def validate_config(config_path: Path) -> bool:
    ...
    try:
        ...
        if not runtime.get('enable_heartbeat', False):
            print(f"   ❌ Heartbeat monitoring not enabled")
            return False
        
        if not runtime.get('enable_progress_tracking', False):
            print(f"   ❌ Progress tracking not enabled")
            return False
        
        # Check compression settings
        compression = config['compression']
        if not compression.get('candidate_policies', []):
            print(f"   ❌ No candidate policies specified")
            return False
        
        if not compression.get('fast_mode', False):
            print(f"   ❌ Fast mode not enabled")
            return False
        ...
        if seeds and train_seed:
            print(f"   ❌ Both compression.seeds and train.seed specified - ambiguous")
            return False
        ...
        if seeds:
            ...
            print(f"   ❌ No seed configuration found")
            return False
